...
func newDaemonStartCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&httpAddr, "http-addr", "", "Expose the daemon API on this loopback HTTP address, e.g. 127.0.0.1:0. Empty uses the default ephemeral loopback port.")
...
func newDaemonRestartCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&httpAddr, "http-addr", "", "Expose the restarted daemon API on this loopback HTTP address, e.g. 127.0.0.1:0. Empty keeps or uses the default ephemeral loopback port.")
	cmd.Flags().BoolVarP(&quiet, "quiet", "q", false, "Suppress output and use only the exit code. Requires detached mode.")
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit machine-readable JSON. Requires detached mode.")
...
func newDaemonStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runDaemonStatus(cmd, target, jsonOut, quiet, commands, operatorCommandScopeFromCommand(cmd, target, rootRepoFlagName), daemonStatusOptions{
				Wait:     wait,
				Down:     down,
				Timeout:  timeout,
				Interval: interval,
