...
func newJobCreateCmd() *cobra.Command {
			...
			if dryRun {
				...
				if dispatchNow {
					if len(j.Steps) > 0 {
						...
						if commands {
							return renderJobCreateApplyCommand(cmd.OutOrStdout(), true, commandOptions)
						}
	...
	cmd.Flags().StringVar(&runtimeKind, "runtime", "", "Runtime profile for --dispatch (claude, codex, or docker). Overrides env and repo config.")
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job create apply command.")
...
func newJobLsCmd() *cobra.Command {
	...
	cmd.Flags().StringSliceVar(&runtimeFilters, "runtime", nil, "Filter by owning instance runtime: claude, codex, or docker. Can repeat or comma-separate.")
...
func newJobShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:  "agent-team job show",
				Commands: commands,
				Conflicts: []commandsModeConflict{
					commandsModeConflicts("--json", jsonOut),
					commandsModeConflicts("--format", format != ""),
					commandsModeConflicts("--events", includeEvents),
				},
			}); err != nil {
				return err
			}
...
func newJobDispatchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job dispatch",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
	...
	cmd.Flags().StringVar(&runtimeKind, "runtime", "", "Runtime profile for the dispatched instance (claude, codex, or docker). Overrides env and repo config.")
...
func newJobUnblockCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job unblock",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
			...
			if dryRun {
				if commands {
					return renderJobUnblockApplyCommand(cmd.OutOrStdout(), true, jobUnblockApplyCommandOptions{
						JobID:             j.ID,
						Repo:              repo,
						RepoSet:           cmd.Flags().Changed("repo"),
						From:              from,
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job unblock apply command when the preview has actionable work.")
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit the updated job or dry-run preview as JSON.")
	cmd.Flags().StringVar(&format, "format", "", "Render the updated job or dry-run preview with a Go template, e.g. '{{.ID}} {{.Status}}'.")
	return cmd
}
...
func newJobTimeoutCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job timeout",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobReleaseCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job release",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobCleanupCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job cleanup",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobExplainCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:  "agent-team job explain",
				Commands: commands,
				Conflicts: []commandsModeConflict{
					commandsModeConflicts("--json", jsonOut),
					commandsModeConflicts("--format", format != ""),
...
func newJobApproveCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job approve",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobAdvanceCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job advance",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
			...
			if dryRun {
				...
				if commands {
					return renderJobAdvanceApplyCommand(cmd.OutOrStdout(), preview.Step != nil, jobAdvanceApplyCommandOptions{
						JobID:          j.ID,
						Repo:           repo,
						RepoSet:        cmd.Flags().Changed("repo"),
						Workspace:      workspace,
	...
	cmd.Flags().StringVar(&runtimeKind, "runtime", "", "Runtime profile for the advanced step dispatch (claude, codex, or docker). Overrides env and repo config.")
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job advance apply command when the preview has actionable work.")
...
func newJobReconcileEventsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job reconcile events",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
			...
			results, err := reconcileSelectedJobsFromEvents(teamDir, jobs, dryRun, time.Now().UTC())
			if err != nil {
				return err
			}
			if commands {
...
func newJobReconcileStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job reconcile status",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobReconcileQueueCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job reconcile queue",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func newJobReconcileGitHubCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateCommandsMode(cmd, commandsModeValidation{
				Command:       "agent-team job reconcile github",
				Commands:      commands,
				RequireDryRun: true,
				DryRun:        dryRun,
				Conflicts: []commandsModeConflict{
...
func validateJobLifecycleRenderOptions(cmd *cobra.Command, label string, dryRun, wait, commands, jsonOut, quiet bool, format *template.Template) error {
	...
	if err := validateCommandsMode(cmd, commandsModeValidation{
		...
		Conflicts: []commandsModeConflict{
			...
			commandsModeConflicts("--format", format != nil),
		},
	}); err != nil {
		return err
	}
...
func runJobQueueListCommands(w io.Writer, teamDir string, j *job.Job, filters queueListFilters, opts queueListOptions, scope operatorCommandScope) error {
	items, err := queueItemsForJob(teamDir, j)
	if err != nil {
		return err
	}
	...
	filtered = prepareQueueListItems(filtered, opts, runtimeByInstance)
	return renderQueueItemsCommands(w, filtered, jobQueueActionResolver(j.ID), scope)
}
...
type jobAdvanceApplyCommandOptions struct {
	JobID          string
	Repo           string
	RepoSet        bool
	Workspace      string
...
func jobReconcileApplyCommandArgs(opts jobReconcileApplyCommandOptions) []string {
	...
	if opts.TargetSet && strings.TrimSpace(opts.TargetAgent) != "" {
		args = append(args, "--target-agent", opts.TargetAgent)
	}
	return args
}
...
func renderJobCreateApplyCommand(w io.Writer, hasAction bool, opts jobCreateApplyCommandOptions) error {
	if !hasAction {
		return nil
	}
	_, err := fmt.Fprintln(w, strings.Join(shellQuoteArgs(jobCreateApplyCommandArgs(opts)), " "))
	return err
}

func jobCreateApplyCommandArgs(opts jobCreateApplyCommandOptions) []string {
	args := []string{"agent-team", "job", "create", opts.Ticket}
...
func renderJobAdvanceApplyCommand(w io.Writer, hasAction bool, opts jobAdvanceApplyCommandOptions) error {
	...
	_, err := fmt.Fprintln(w, strings.Join(shellQuoteArgs(jobAdvanceApplyCommandArgs(opts)), " "))
	return err
}

func jobAdvanceApplyCommandArgs(opts jobAdvanceApplyCommandOptions) []string {
	args := []string{"agent-team", "job", "advance", opts.JobID}
	if opts.RepoSet && strings.TrimSpace(opts.Repo) != "" {
		args = append(args, "--repo", opts.Repo)
	}
	if opts.WorkspaceSet && strings.TrimSpace(opts.Workspace) != "" {
...
type jobUnblockApplyCommandOptions struct {
	JobID             string
	Repo              string
	RepoSet           bool
	From              string
...
func renderJobUnblockApplyCommand(w io.Writer, hasAction bool, opts jobUnblockApplyCommandOptions) error {
	...
	_, err := fmt.Fprintln(w, strings.Join(shellQuoteArgs(jobUnblockApplyCommandArgs(opts)), " "))
	return err
}

func jobUnblockApplyCommandArgs(opts jobUnblockApplyCommandOptions) []string {
	args := []string{"agent-team", "job", "unblock", opts.JobID}
	if opts.RepoSet && strings.TrimSpace(opts.Repo) != "" {
		args = append(args, "--repo", opts.Repo)
	}
	if opts.FromSet && strings.TrimSpace(opts.From) != "" {
