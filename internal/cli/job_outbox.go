...
func newJobOutboxRetryCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox retry: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox retry: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			if format != "" && jsonOut {
			...
			if retryAll {
				...
				if commands {
					results, err := jobOutboxRetryAllResults(teamDir, j, filters, outboxListOptions{Sort: sortMode, Limit: limit}, true)
					if err != nil {
						return err
					}
					return renderOutboxApplyCommand(cmd.OutOrStdout(), outboxActionResultsHaveDryRunAction(results, "would_retry"), outboxApplyCommandOptions{
						BaseArgs: []string{"agent-team", "job", "outbox", "retry", args[0]},
						Repo:     repo,
			...
			if commands {
				return renderOutboxApplyCommand(cmd.OutOrStdout(), true, outboxApplyCommandOptions{
					BaseArgs: []string{"agent-team", "job", "outbox", "retry", args[0], args[1]},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job outbox retry apply command when the preview has actionable work.")
...
func newJobOutboxDropCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if commands && !dryRun {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox drop: --commands requires --dry-run.")
				return exitErr(2)
			}
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox drop: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox drop: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			if format != "" && jsonOut {
			...
			if dropAll {
				...
				if commands {
					results, err := jobOutboxDropAllResults(teamDir, j, filters, outboxListOptions{Sort: sortMode, Limit: limit}, true)
					if err != nil {
						return err
					}
					return renderOutboxApplyCommand(cmd.OutOrStdout(), outboxActionResultsHaveDryRunAction(results, "would_drop"), outboxApplyCommandOptions{
						BaseArgs: []string{"agent-team", "job", "outbox", "drop", args[0]},
						Repo:     repo,
						RepoSet:  cmd.Flags().Changed("repo"),
						All:      true,
						State:    stateFilter,
			...
			if commands {
				return renderOutboxApplyCommand(cmd.OutOrStdout(), true, outboxApplyCommandOptions{
					BaseArgs: []string{"agent-team", "job", "outbox", "drop", args[0], args[1]},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job outbox drop apply command when the preview has actionable work.")
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit machine-readable JSON.")
...
func newJobOutboxPruneCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if commands && !dryRun {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox prune: --commands requires --dry-run.")
				return exitErr(2)
			}
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox prune: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox prune: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			if format != "" && jsonOut {
			...
			if commands {
				results, err := jobOutboxPruneResults(teamDir, j, state, olderThan, time.Now().UTC(), true, filters, limit)
				if err != nil {
					return err
				}
				return renderOutboxApplyCommand(cmd.OutOrStdout(), outboxPruneResultsHaveDryRunAction(results), outboxApplyCommandOptions{
					BaseArgs:     []string{"agent-team", "job", "outbox", "prune", args[0]},
					Repo:         repo,
					RepoSet:      cmd.Flags().Changed("repo"),
					State:        stateFlag,
					StateSet:     cmd.Flags().Changed("state"),
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job outbox prune apply command when the preview has actionable work.")
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if summary && (cmd.Flags().Changed("sort") || limit > 0) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine: --sort and --limit cannot be combined with --summary.")
				return exitErr(2)
			}
...
func newJobOutboxQuarantineShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine show: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine show: --commands cannot be combined with --format.")
				return exitErr(2)
			}
...
func newJobOutboxQuarantineRestoreCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if commands && !dryRun {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine restore: --commands requires --dry-run.")
				return exitErr(2)
			}
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine restore: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine restore: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			...
			if restoreAll {
				...
				if commands {
					return renderOutboxApplyCommand(cmd.OutOrStdout(), outboxQuarantineRestoreResultsHaveDryRunAction(results, "would_restore"), outboxApplyCommandOptions{
						BaseArgs: []string{"agent-team", "job", "outbox", "quarantine", "restore", j.ID},
						Repo:     repo,
						RepoSet:  cmd.Flags().Changed("repo"),
						All:      true,
						Force:    force,
			...
			if commands {
				return renderOutboxApplyCommand(cmd.OutOrStdout(), result.DryRun && result.Action == "would_restore", outboxApplyCommandOptions{
					BaseArgs: []string{"agent-team", "job", "outbox", "quarantine", "restore", j.ID, result.Path},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
					Force:    force,
				})
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job outbox quarantine restore apply command when the preview has actionable work.")
...
func newJobOutboxQuarantineDropCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if commands && !dryRun {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine drop: --commands requires --dry-run.")
				return exitErr(2)
			}
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine drop: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team job outbox quarantine drop: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			if olderThan < 0 {
			...
			if dropAll {
				...
				if commands {
					return renderOutboxApplyCommand(cmd.OutOrStdout(), outboxQuarantineDropResultsHaveDryRunAction(results, "would_drop"), outboxApplyCommandOptions{
						BaseArgs:     []string{"agent-team", "job", "outbox", "quarantine", "drop", j.ID},
						Repo:         repo,
						RepoSet:      cmd.Flags().Changed("repo"),
						All:          true,
						State:        stateFilter,
			...
			if commands {
				return renderOutboxApplyCommand(cmd.OutOrStdout(), result.DryRun && result.Action == "would_drop", outboxApplyCommandOptions{
					BaseArgs: []string{"agent-team", "job", "outbox", "quarantine", "drop", j.ID, result.Path},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching job outbox quarantine drop apply command when the preview has actionable work.")
...
func runJobOutboxRetryAll(w io.Writer, teamDir string, j *job.Job, filters outboxListFilters, opts outboxListOptions, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := jobOutboxRetryAllResults(teamDir, j, filters, opts, dryRun)
	if err != nil {
		return err
	}
...
func jobOutboxRetryAllResults(teamDir string, j *job.Job, filters outboxListFilters, opts outboxListOptions, dryRun bool) ([]outboxActionResult, error) {
...
func runJobOutboxDropAll(w io.Writer, teamDir string, j *job.Job, filters outboxListFilters, opts outboxListOptions, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := jobOutboxDropAllResults(teamDir, j, filters, opts, dryRun)
	if err != nil {
		return err
	}
	return renderOutboxActionResults(w, results, jsonOut, tmpl)
...
func jobOutboxDropAllResults(teamDir string, j *job.Job, filters outboxListFilters, opts outboxListOptions, dryRun bool) ([]outboxActionResult, error) {
	matches, err := filteredOutboxItemsForJob(teamDir, j, filters, opts)
	if err != nil {
		return nil, err
	}
...
func runJobOutboxPrune(w io.Writer, teamDir string, j *job.Job, state string, olderThan time.Duration, now time.Time, dryRun bool, filters outboxListFilters, limit int, jsonOut bool, tmpl *template.Template) error {
	results, err := jobOutboxPruneResults(teamDir, j, state, olderThan, now, dryRun, filters, limit)
	if err != nil {
		return err
	}
...
func jobOutboxPruneResults(teamDir string, j *job.Job, state string, olderThan time.Duration, now time.Time, dryRun bool, filters outboxListFilters, limit int) ([]outboxPruneResult, error) {
	items, err := outboxItemsForJob(teamDir, j)
	if err != nil {
		return nil, err
	}
