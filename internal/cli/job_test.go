...
func writeJobAuthorityTopology(t *testing.T, teamDir string) {
	t.Helper()
	body := topoFixture + `
...
func setJobAuthorityOriginEnv(t *testing.T, agent, jobID string) {
	...
	t.Setenv("AGENT_TEAM_INSTANCE", agent+"-"+jobID)
	t.Setenv("AGENT_TEAM_ORIGIN_INSTANCE", "")
	t.Setenv("AGENT_TEAM_ORIGIN_AGENT", agent)
	t.Setenv("AGENT_TEAM_JOB_ID", jobID)
	t.Setenv("AGENT_TEAM_ORIGIN_JOB", "")
	t.Setenv("AGENT_TEAM_ORIGIN_TRIGGER", "pipeline:ticket_to_pr:test")
	t.Setenv("AGENT_TEAM_ORIGIN_BUILD", "")
}

func jobAuthorityViolationEvents(t *testing.T, teamDir, jobID string) []job.Event {
	t.Helper()
...
func daemonAuthorityViolationEvents(t *testing.T, teamDir string) []*daemon.LifecycleEvent {
	t.Helper()
	events, err := daemon.ListLifecycleEvents(daemon.DaemonRoot(teamDir))
	if err != nil {
		t.Fatalf("lifecycle events: %v", err)
	}
	var out []*daemon.LifecycleEvent
	for _, ev := range events {
		if ev.Action == "authority_violation" {
			out = append(out, ev)
		}
	}
	return out
...
func TestJobCreateListShowClose(t *testing.T) {
	...
	duplicate.SetArgs([]string{"job", "create", "SQU-42", "--target", "worker", "--repo", tmp})
	var dupCode ExitCode
	if err := duplicate.Execute(); err == nil {
		t.Fatalf("job create duplicate succeeded")
	} else if !errors.As(err, &dupCode) || dupCode != 1 {
		t.Fatalf("job create duplicate exit = %v, want 1; stderr=%s", err, dupErr.String())
	}
	for _, want := range []string{`job "squ-42" already exists`, `status "queued"`, "--id", "clean up"} {
	...
	duplicateJSON.SetArgs([]string{"job", "create", "SQU-42", "--target", "worker", "--repo", tmp, "--json"})
	var dupJSONCode ExitCode
	if err := duplicateJSON.Execute(); err == nil {
		t.Fatalf("job create duplicate json succeeded")
	} else if !errors.As(err, &dupJSONCode) || dupJSONCode != 1 {
		t.Fatalf("job create duplicate json exit = %v, want 1; stderr=%s", err, dupJSONErr.String())
	}
	...
	if conflict.Error != "job_conflict" || conflict.JobID != "squ-42" || conflict.Status != "queued" || !strings.Contains(conflict.Suggestion, "--id") {
...
func TestJobBounceRequeuesCompletedStepAndAudits(t *testing.T) {
	...
	j := &job.Job{
		...
		Branch:    "squ-162-repair",
		Worktree:  filepath.Join(tmp, ".claude", "worktrees", "squ-162-repair"),
		CreatedAt: now,
		UpdatedAt: now,
		Steps: []job.Step{
			...
			{ID: "approve", Target: "manager", Status: job.StatusBlocked, After: []string{"review"}, Gate: job.StepGateManual, ApprovalRequired: true, ApprovalID: "old-approval", ApprovalStatus: job.ApprovalStatusApproved},
		},
	}
	if err := job.Write(teamDir, j); err != nil {
		t.Fatalf("write job: %v", err)
	...
	if preview.Job.Attempt != 2 || preview.Job.Head != "" || preview.Job.Steps[2].ApprovalID != "" || preview.Job.Steps[2].ApprovalStatus != "" {
		t.Fatalf("preview retained prior-attempt state: %+v", preview.Job)
	}
	...
	if unchanged.Attempt != 1 || unchanged.Head == "" || unchanged.Steps[2].ApprovalID != "old-approval" {
		t.Fatalf("dry-run mutated attempt state = %+v", unchanged)
	}
	...
	if bounced.Attempt != 2 || bounced.Head != "" || bounced.Branch != "squ-162-repair" || bounced.Worktree != j.Worktree || bounced.Steps[2].ApprovalID != "" || bounced.Steps[2].ApprovalStatus != "" {
		t.Fatalf("bounce attempt state = %+v", bounced)
	}
...
func TestJobStepRejectsStaleAttemptCompletionWithReusedInstance(t *testing.T) {
	root := t.TempDir()
	initInto(t, root)
	teamDir := filepath.Join(root, ".agent_team")
	now := time.Now().UTC()
	j := &job.Job{
		ID: "squ-230", Ticket: "SQU-230", Target: "worker", Pipeline: "ticket_to_pr",
		Attempt: 2, Status: job.StatusRunning, Branch: "squ-230-repair", Worktree: filepath.Join(root, "repair-worktree"),
		CreatedAt: now, UpdatedAt: now,
		Steps: []job.Step{
			{ID: "implement", Target: "worker", Status: job.StatusRunning, Instance: "worker-squ-230-implement"},
			{ID: "verify", Target: "verifier", Status: job.StatusBlocked, After: []string{"implement"}},
		},
	}
	if err := job.Write(teamDir, j); err != nil {
		t.Fatal(err)
	...
	stale := NewRootCmd()
	staleErr := &bytes.Buffer{}
	stale.SetErr(staleErr)
	stale.SetArgs([]string{"job", "step", j.ID, "implement", "--repo", root, "--status", "done", "--instance", "worker-squ-230-implement", "--attempt", "1", "--head", strings.Repeat("a", 40)})
	if err := stale.Execute(); err == nil || !strings.Contains(staleErr.String(), "stale attempt 1") {
		t.Fatalf("stale completion err=%v stderr=%q", err, staleErr.String())
	}
	unchanged, err := job.Read(teamDir, j.ID)
	if err != nil {
		t.Fatal(err)
	}
	if unchanged.Steps[0].Status != job.StatusRunning || unchanged.Head != "" || unchanged.Branch != j.Branch || unchanged.Worktree != j.Worktree {
		t.Fatalf("stale completion mutated job = %+v", unchanged)
	}

	current := NewRootCmd()
	current.SetArgs([]string{"job", "step", j.ID, "implement", "--repo", root, "--status", "done", "--instance", "worker-squ-230-implement", "--attempt", "2", "--head", strings.Repeat("b", 40)})
	...
	if updated.Steps[0].Status != job.StatusDone || updated.Steps[1].Status != job.StatusBlocked || updated.Head != strings.Repeat("b", 40) {
		t.Fatalf("current completion = %+v", updated)
	}
}
...
func TestJobApproveRequiresCurrentAttemptExactHeadVerifierAndReviewerEvidence(t *testing.T) {
	...
	head := strings.Repeat("b", 40)
	j := &job.Job{
		ID: "squ-231", Ticket: "SQU-231", Target: "worker", Pipeline: "ticket_to_pr",
	...
	appendGate := func(attempt int, step, commit string) {
		t.Helper()
		if err := job.AppendGateRecord(teamDir, &job.GateRecord{TS: time.Now().UTC(), JobID: j.ID, Attempt: attempt, Step: step, Commit: commit, Name: step, Status: job.GateStatusPass}); err != nil {
			t.Fatal(err)
		}
	}
	appendGate(1, "verify", strings.Repeat("a", 40))
	appendGate(1, "review", strings.Repeat("a", 40))

	approve := func(want string) error {
		t.Helper()
		cmd := NewRootCmd()
		stderr := &bytes.Buffer{}
		cmd.SetErr(stderr)
		...
		if want != "" && !strings.Contains(stderr.String(), want) {
			t.Fatalf("approve stderr=%q, want %q", stderr.String(), want)
		}
		return err
	}
	if err := approve(`step "verify" lacks passing gate evidence`); err == nil {
		t.Fatal("approval accepted prior-attempt evidence")
	}
	appendGate(2, "verify", head)
	appendGate(2, "review", strings.Repeat("c", 40))
	if err := approve(`step "review" lacks passing gate evidence`); err == nil {
		t.Fatal("approval accepted wrong-head review evidence")
	}
	appendGate(2, "review", head)
	if err := approve(""); err != nil {
		t.Fatalf("approval with current evidence: %v", err)
	}
	updated, err := job.Read(teamDir, j.ID)
	if err != nil {
		t.Fatal(err)
	}
	if updated.Steps[3].Status != job.StatusQueued {
		t.Fatalf("approval step = %+v", updated.Steps[3])
	}
}
...
func TestJobHeadChangeInvalidatesDownstreamOwnershipAndApproval(t *testing.T) {
	now := time.Now().UTC()
	j := &job.Job{
		ID: "gh-230", Attempt: 2, Head: strings.Repeat("a", 40), Status: job.StatusDone,
		Steps: []job.Step{
			{ID: "implement", Target: "worker", Status: job.StatusDone, Instance: "worker-gh-230"},
			{ID: "verify", Target: "verifier", Status: job.StatusDone, Instance: "verifier-gh-230", After: []string{"implement"}, StartedAt: now, FinishedAt: now},
			{ID: "review", Target: "reviewer", Status: job.StatusDone, Instance: "reviewer-gh-230", After: []string{"verify"}, StartedAt: now, FinishedAt: now},
			{ID: "approve", Target: "manager", Status: job.StatusDone, After: []string{"review"}, Gate: job.StepGateManual, ApprovalID: "old", ApprovalStatus: job.ApprovalStatusApproved},
		},
	}
	if err := updateJobStep(j, "implement", job.StatusDone, jobStepUpdate{Head: strings.Repeat("b", 40)}); err != nil {
		t.Fatal(err)
	}
	if j.Head != strings.Repeat("b", 40) || j.Steps[0].Status != job.StatusDone {
		t.Fatalf("implementation/head = %+v", j)
	}
	for _, step := range j.Steps[1:] {
		if step.Status != job.StatusBlocked || step.Instance != "" || !step.StartedAt.IsZero() || !step.FinishedAt.IsZero() || step.ApprovalID != "" || step.ApprovalStatus != "" {
			t.Fatalf("downstream step retained old-head state: %+v", step)
		}
	}
}
...
func TestJobBounceAdvanceDispatchesRequeuedStep(t *testing.T) {
	...
	if payload["pipeline_step"] != "implement" || payload["workspace"] != "repo" || payload["attempt"] != float64(2) || !strings.Contains(kickoff, "## Review findings (bounce 1)") || !strings.Contains(kickoff, "repair the edge case") {
...
func TestJobWaitPollsUntilNextStepState(t *testing.T) {
	...
	anyStep := NewRootCmd()
	anyStepOut, anyStepErr := &bytes.Buffer{}, &bytes.Buffer{}
	anyStep.SetOut(anyStepOut)
	anyStep.SetErr(anyStepErr)
	anyStep.SetArgs([]string{"job", "wait", "squ-64", "--repo", tmp, "--next-state", "all", "--step", "implement", "--timeout", "2s", "--interval", "10ms", "--format", "{{.ID}} {{.Status}}"})
	if err := anyStep.Execute(); err != nil {
		t.Fatalf("job wait next-state all: %v\nstderr=%s", err, anyStepErr.String())
	}
	if got := strings.TrimSpace(anyStepOut.String()); got != "squ-64 running" {
		t.Fatalf("next-state all output = %q", got)
	}
	...
	go func() {
		...
		updated.LastEvent = "implement_done"
		updated.UpdatedAt = time.Now().UTC()
		if err := job.Write(teamDir, updated); err != nil {
			t.Errorf("write ready next step: %v", err)
		}
	}()
	...
	cmd.SetOut(out)
	cmd.SetErr(stderr)
	cmd.SetArgs([]string{"job", "wait", "squ-64", "--repo", tmp, "--next-state", "ready", "--step", "review", "--timeout", "2s", "--interval", "10ms", "--json"})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("job wait next state: %v\nstderr=%s", err, stderr.String())
	}
	<-done
	var got job.Job
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode wait next-state json: %v\nbody=%s", err, out.String())
	}
	if got.ID != "squ-64" || got.Steps[0].Status != job.StatusDone || got.LastEvent != "implement_done" {
		t.Fatalf("wait next-state result = %+v", got)
	}
}
...
func TestJobWaitNextStateTimesOut(t *testing.T) {
	tmp := t.TempDir()
	initInto(t, tmp)
	teamDir := filepath.Join(tmp, ".agent_team")
	now := time.Now().UTC()
	...
	cmd.SetOut(out)
	cmd.SetErr(stderr)
	cmd.SetArgs([]string{"job", "wait", "squ-65", "--repo", tmp, "--next-state", "ready", "--step", "review", "--timeout", "1ms", "--interval", "10ms"})
	if err := cmd.Execute(); err == nil {
		t.Fatalf("job wait next-state succeeded unexpectedly")
	}
	for _, want := range []string{"next-state=ready", "step=review", "current=running", "next_state=queued", "step=implement"} {
	...
	badState.SetArgs([]string{"job", "wait", "squ-65", "--repo", tmp, "--next-state", "stuck"})
	if err := badState.Execute(); err == nil {
		t.Fatalf("job wait bad next-state succeeded")
	}
	if !strings.Contains(badStateErr.String(), "--next-state must be ready, queued, running, blocked, failed, held, done, none, or all") {
...
func TestJobPsScopesRowsAndSteps(t *testing.T) {
	tmp := t.TempDir()
	initInto(t, tmp)
	teamDir := filepath.Join(tmp, ".agent_team")
	now := time.Now().UTC()
	if err := job.Write(teamDir, &job.Job{
		...
		Ticket:    "SQU-119",
		Target:    "manager",
		Instance:  "manager-squ-119",
		Pipeline:  "ticket_to_pr",
		Status:    job.StatusRunning,
		CreatedAt: now,
		UpdatedAt: now,
		Steps: []job.Step{
			{ID: "implement", Target: "worker", Status: job.StatusRunning, Instance: "worker-squ-119-implement", StartedAt: now.Add(-30 * time.Minute)},
			{ID: "review", Target: "manager", Status: job.StatusBlocked, Instance: "reviewer-squ-119-review", StartedAt: now.Add(-20 * time.Minute)},
		},
	}); err != nil {
		t.Fatalf("write job: %v", err)
	}
	for _, meta := range []*daemon.Metadata{
		{Instance: "manager-squ-119", Job: "squ-119", Agent: "manager", Runtime: string(runtimebin.KindClaude), Status: daemon.StatusRunning, PID: os.Getpid(), Workspace: tmp, StartedAt: now.Add(-40 * time.Minute)},
		{Instance: "worker-squ-119-implement", Agent: "worker", Runtime: string(runtimebin.KindCodex), Status: daemon.StatusRunning, PID: os.Getpid(), Workspace: tmp, StartedAt: now.Add(-30 * time.Minute)},
		{Instance: "reviewer-squ-119-review", Agent: "manager", Runtime: string(runtimebin.KindCodex), Status: daemon.StatusStopped, PID: os.Getpid(), Workspace: tmp, StartedAt: now.Add(-20 * time.Minute), StoppedAt: now.Add(-10 * time.Minute)},
		{Instance: "sidecar-squ-119", Job: "squ-119", Agent: "worker", Runtime: string(runtimebin.KindCodex), Status: daemon.StatusRunning, PID: os.Getpid(), Workspace: tmp, StartedAt: now.Add(-15 * time.Minute)},
	...
	allRows := NewRootCmd()
	allRowsOut, allRowsErr := &bytes.Buffer{}, &bytes.Buffer{}
	allRows.SetOut(allRowsOut)
	allRows.SetErr(allRowsErr)
	allRows.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--json"})
	if err := allRows.Execute(); err != nil {
		t.Fatalf("job ps json: %v\nstderr=%s", err, allRowsErr.String())
	}
	var rows []psJSONRow
	if err := json.Unmarshal(allRowsOut.Bytes(), &rows); err != nil {
		t.Fatalf("decode job ps: %v\nbody=%s", err, allRowsOut.String())
	}
	if got := psJSONRowNames(rows); strings.Join(got, ",") != "manager-squ-119,reviewer-squ-119-review,sidecar-squ-119,worker-squ-119-implement" {
		t.Fatalf("job ps rows = %v", got)
	}
	...
	running.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--status", "running", "--json"})
	if err := running.Execute(); err != nil {
		t.Fatalf("job ps running: %v\nstderr=%s", err, runningErr.String())
	}
	rows = nil
	if err := json.Unmarshal(runningOut.Bytes(), &rows); err != nil {
		t.Fatalf("decode job ps running: %v\nbody=%s", err, runningOut.String())
	}
	if got := psJSONRowNames(rows); strings.Join(got, ",") != "manager-squ-119,sidecar-squ-119,worker-squ-119-implement" {
		t.Fatalf("job ps running rows = %v", got)
	}
	...
	codex.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--runtime", "codex", "--format", "{{.Instance}}"})
	if err := codex.Execute(); err != nil {
		t.Fatalf("job ps runtime format: %v\nstderr=%s", err, codexErr.String())
	}
	if got, want := strings.TrimSpace(codexOut.String()), "reviewer-squ-119-review\nsidecar-squ-119\nworker-squ-119-implement"; got != want {
		t.Fatalf("job ps runtime format = %q, want %q", got, want)
	}
	...
	step.SetOut(stepOut)
	step.SetErr(stepErr)
	step.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--step", "review", "--json"})
	if err := step.Execute(); err != nil {
		t.Fatalf("job ps step: %v\nstderr=%s", err, stepErr.String())
	}
	rows = nil
	if err := json.Unmarshal(stepOut.Bytes(), &rows); err != nil {
		t.Fatalf("decode job ps step: %v\nbody=%s", err, stepOut.String())
	}
	if got := psJSONRowNames(rows); strings.Join(got, ",") != "reviewer-squ-119-review" {
		t.Fatalf("job ps step rows = %v", got)
	}
	if rows[0].Status != string(daemon.StatusStopped) {
		t.Fatalf("job ps step status = %+v", rows[0])
	}
	...
	quiet.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--quiet", "--runtime", "codex", "--status", "running"})
	if err := quiet.Execute(); err != nil {
		t.Fatalf("job ps quiet: %v\nstderr=%s", err, quietErr.String())
	}
	if got, want := strings.TrimSpace(quietOut.String()), "sidecar-squ-119\nworker-squ-119-implement"; got != want {
		t.Fatalf("job ps quiet = %q, want %q", got, want)
	}
	...
	summary.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--summary", "--json"})
	if err := summary.Execute(); err != nil {
		t.Fatalf("job ps summary: %v\nstderr=%s", err, summaryErr.String())
	}
	var counts psSummaryJSON
	if err := json.Unmarshal(summaryOut.Bytes(), &counts); err != nil {
		t.Fatalf("decode job ps summary: %v\nbody=%s", err, summaryOut.String())
	}
	if counts.Total != 4 || counts.Running != 3 || counts.Stopped != 1 {
		t.Fatalf("job ps summary = %+v", counts)
	}
	...
	invalidStep.SetArgs([]string{"job", "ps", "SQU-119", "--repo", tmp, "--step", "deploy"})
	if err := invalidStep.Execute(); err == nil {
		t.Fatal("job ps accepted unknown step")
	}
...
func TestJobNoteDryRunDoesNotMutateJob(t *testing.T) {
	...
	commands.SetOut(commandsOut)
	commands.SetErr(commandsErr)
	commands.SetArgs([]string{"job", "note", "squ-74", "--repo", tmp, "--actor", "ops", "--message", "preview note", "--dry-run", "--commands"})
	if err := commands.Execute(); err != nil {
		t.Fatalf("job note dry-run commands: %v\nstderr=%s", err, commandsErr.String())
	}
	wantCommand := strings.Join(shellQuoteArgs([]string{
		"agent-team", "job", "note", "squ-74",
		"--repo", tmp,
		"--actor", "ops",
		"--message", "preview note",
	}), " ")
	if got := strings.TrimSpace(commandsOut.String()); got != wantCommand {
		t.Fatalf("job note dry-run commands = %q, want %q", got, wantCommand)
	}
...
func TestJobNoteCommandValidation(t *testing.T) {
	cases := []struct {
		args []string
		want string
	}{
...
func TestJobBlockDryRunDoesNotMutateJob(t *testing.T) {
	...
	commands.SetOut(commandsOut)
	commands.SetErr(commandsErr)
	commands.SetArgs([]string{"job", "block", "squ-76", "--repo", tmp, "--actor", "linear", "--message", "preview block", "--dry-run", "--commands"})
	if err := commands.Execute(); err != nil {
		t.Fatalf("job block dry-run commands: %v\nstderr=%s", err, commandsErr.String())
	}
	wantCommand := strings.Join(shellQuoteArgs([]string{
		"agent-team", "job", "block", "squ-76",
		"--repo", tmp,
		"--actor", "linear",
		"--message", "preview block",
	}), " ")
	if got := strings.TrimSpace(commandsOut.String()); got != wantCommand {
		t.Fatalf("job block dry-run commands = %q, want %q", got, wantCommand)
	}
...
func TestJobBlockCommandValidation(t *testing.T) {
	cases := []struct {
		args []string
		want string
	}{
...
func TestJobStepAdvanceAllowedUnderEnforcedAuthority(t *testing.T) {
	cases := []struct {
		name         string
		actor        string
		jobID        string
		...
		{
			name:         "worker implement advances",
			actor:        "worker",
			jobID:        "squ-209-worker",
			completeStep: "implement",
			steps: []job.Step{
				{ID: "implement", Target: "worker", Status: job.StatusRunning, Instance: "worker-squ-209-worker-implement"},
				{ID: "review", Target: "reviewer", Status: job.StatusBlocked, After: []string{"implement"}},
			},
		},
		{
			name:         "verifier verify advances",
			actor:        "verifier",
			jobID:        "squ-209-verifier",
			completeStep: "verify",
			steps: []job.Step{
				{ID: "implement", Target: "worker", Status: job.StatusDone, Instance: "worker-squ-209-verifier-implement"},
				{ID: "verify", Target: "verifier", Status: job.StatusRunning, Instance: "verifier-squ-209-verifier-verify", After: []string{"implement"}},
				{ID: "review", Target: "reviewer", Status: job.StatusBlocked, After: []string{"verify"}},
			},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			initInto(t, root)
			teamDir := filepath.Join(root, ".agent_team")
			topologyPath := filepath.Join(teamDir, "instances.toml")
			topologyText, err := os.ReadFile(topologyPath)
			if err != nil {
				t.Fatalf("read topology: %v", err)
			}
			topologyText = []byte(strings.Replace(string(topologyText), `enforcement = "audit"`, `enforcement = "enforce"`, 1))
			if err := os.WriteFile(topologyPath, topologyText, 0o644); err != nil {
			...
			now := time.Now().UTC()
			steps := append([]job.Step(nil), tc.steps...)
			for i := range steps {
				if steps[i].StartedAt.IsZero() && steps[i].Status != job.StatusBlocked {
					steps[i].StartedAt = now.Add(-30 * time.Minute)
				}
				if steps[i].FinishedAt.IsZero() && steps[i].Status == job.StatusDone {
					steps[i].FinishedAt = now.Add(-15 * time.Minute)
				}
			}
			j := &job.Job{
				ID:        tc.jobID,
				Ticket:    strings.ToUpper(tc.jobID),
				Target:    "worker",
				Kickoff:   "authorized step advance",
				Pipeline:  "ticket_to_pr",
				Status:    job.StatusRunning,
				CreatedAt: now,
				UpdatedAt: now,
			...
			cmd.SetOut(out)
			cmd.SetErr(stderr)
			cmd.SetArgs([]string{"job", "step", j.ID, tc.completeStep, "--status", "done", "--advance", "--workspace", "repo", "--repo", root, "--json"})
			if err := cmd.Execute(); err != nil {
				t.Fatalf("%s job step --advance: %v\nstderr=%s", tc.actor, err, stderr.String())
			}
			var result jobAdvanceResult
			if err := json.Unmarshal(out.Bytes(), &result); err != nil {
				t.Fatalf("decode advance json: %v\nbody=%s", err, out.String())
			}
			wantReviewInstance := "reviewer-" + tc.jobID + "-review"
			if result.Job == nil || result.Job.Status != job.StatusRunning || result.Job.LastEvent != "advance_dispatched" {
				t.Fatalf("advance job = %+v", result.Job)
			}
			if result.Step == nil || result.Step.ID != "review" || result.Step.Status != job.StatusRunning || result.Step.Instance != wantReviewInstance {
				t.Fatalf("advance step = %+v", result.Step)
			}
			...
			stopAndWaitForTest(t, mgr, wantReviewInstance)
		})
	}
}
...
func TestJobGateSetAuthorityAuditAllowlist(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			initInto(t, root)
			teamDir := filepath.Join(root, ".agent_team")
			writeJobAuthorityTopology(t, teamDir)
			j := mustNewJob(t, "SQU-610", "worker")
			if err := job.Write(teamDir, j); err != nil {
				t.Fatalf("write job: %v", err)
			}
			...
			cmd.SetOut(out)
			cmd.SetErr(stderr)
			cmd.SetArgs([]string{"job", "gate", "set", j.ID, "review", "--repo", root, "--status", "pass", "--json"})
			if err := cmd.Execute(); err != nil {
				t.Fatalf("job gate set: %v\nstderr=%s", err, stderr.String())
			}
			records, err := job.ListGateRecords(teamDir, j.ID)
			if err != nil {
				t.Fatalf("list gate records: %v", err)
			}
			if len(records) != 1 || records[0].Name != "review" || records[0].Status != job.GateStatusPass {
				t.Fatalf("gate records = %+v", records)
			}
			jobViolations := jobAuthorityViolationEvents(t, teamDir, j.ID)
			daemonViolations := daemonAuthorityViolationEvents(t, teamDir)
			want := 0
			if tc.wantViolation {
				want = 1
			}
			if len(jobViolations) != want || len(daemonViolations) != want {
				t.Fatalf("violations: job=%+v daemon=%+v want %d", jobViolations, daemonViolations, want)
			}
			if tc.wantViolation {
				ev := jobViolations[0]
				...
				if daemonViolations[0].Origin.Agent != tc.actor || daemonViolations[0].Origin.Team != "platform" {
					t.Fatalf("daemon authority event = %+v", daemonViolations[0])
				}
			}
		})
	}
...
func TestJobMergeAuthorityAuditAllowlist(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			j := mustNewJob(t, "SQU-611", "worker")
			j.PR = "https://github.com/acme/repo/pull/611"
			j.Branch = "worker-squ-611"
			j.Merge = &job.Merge{Strategy: "squash"}
			if err := job.Write(teamDir, j); err != nil {
				t.Fatalf("write job: %v", err)
			}
