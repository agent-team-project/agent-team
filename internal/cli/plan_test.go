...
func TestPlanJSONShowsTopologyAndDaemonMetadata(t *testing.T) {
	...
	if body.Summary.Total != 6 || body.Summary.Start != 1 || body.Summary.Resume != 1 || body.Summary.OnDemand != 3 || body.Summary.Extra != 1 {
...
func TestPlanSummaryJSONShowsAggregateCounts(t *testing.T) {
	...
	if body.Summary.Total != 5 || body.Summary.Actions["start"] != 2 || body.Summary.Actions["on-demand"] != 3 || !body.Summary.DryRun {
		t.Fatalf("summary = %+v, want two starts and two on-demand previews", body.Summary)
	}
	if body.Summary.Statuses["unknown"] != 5 {
		t.Fatalf("statuses = %+v, want unknown=5", body.Summary.Statuses)
	}
}
...
func TestPlanStopExtrasMarksOnlyRunningExtras(t *testing.T) {
	...
	if body.Summary.Total != 7 || body.Summary.Stop != 1 || body.Summary.Extra != 1 {
...
func TestPlanTextRendersTableAndSummary(t *testing.T) {
	...
	for _, want := range []string{
		...
		"start",
		"on-demand",
		"summary: total=5 start=2 resume=0 keep=0 on-demand=3 extra=0",
	} {
