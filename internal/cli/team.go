...
func newTeamRuntimeResumePlanCommand(cfg teamRuntimeResumePlanCommandConfig) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			plans, err := collectTeamRuntimeResumePlans(teamDir, args[0], stepID, statusFilters, runtimeFilter, actionFilters, staleOnly || runtimeStale, unhealthyOnly, capabilityFilters)
			if err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "%s: %v\n", cfg.ErrorName, err)
				return exitErr(1)
			}
...
func collectTeamRuntimeResumePlans(teamDir, name string, stepFilter string, statusFilters []string, runtimeFilters []string, actionFilters []string, staleOnly bool, unhealthyOnly bool, capabilityFilters runtimeResumeCapabilityFilters) ([]runtimeResumePlan, error) {
	top, team, err := loadTopologyTeam(teamDir, name)
	if err != nil {
		return nil, err
	}
...
func newTeamRunCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team run apply command. agent-team follow-ups preserve the selected repo scope.")
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if follow && sortMode == "newest" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team job-events: --sort newest cannot be combined with --follow.")
				return exitErr(2)
			}
			...
			loadJobs := func() ([]*job.Job, error) {
				return collectTeamJobs(teamDir, teamName, jobListFilters{Sort: "id"})
			}
			if follow {
				ctx, stop := signal.NotifyContext(cmd.Context(), os.Interrupt)
				defer stop()
...
func newTeamTimelineCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if summary && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team timeline: --summary cannot be combined with --format.")
				return exitErr(2)
			}
			sortMode, err := parseEventSort(sortBy)
			if err != nil {
			...
			return renderScopedJobTimeline(cmd.OutOrStdout(), "team:"+teamName, entries, jsonOut, summary, tmpl)
		},
	}
	cmd.Flags().StringVar(&repo, "repo", cwd, repoFlagHelp)
...
func newTeamJobWaitCmd() *cobra.Command {
	var (
		repo         string
		jobFilters   []string
		statuses     []string
	...
	cmd := &cobra.Command{
		Use:   "wait-jobs <team>",
		Short: "Wait for team-owned jobs to reach a lifecycle status, event, or next step.",
		Long: "Wait for every selected job owned by one team to reach one of the requested lifecycle statuses, last events, and/or pipeline next-step states. " +
			...
			"Use `team wait` for team-owned instance lifecycle waits.",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if interval < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team wait-jobs: --interval must be >= 0.")
				return exitErr(2)
			}
			if timeout < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team wait-jobs: --timeout must be >= 0.")
				return exitErr(2)
			}
			if quiet && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team wait-jobs: choose one of --quiet or --json.")
				return exitErr(2)
			}
			if format != "" && (quiet || jsonOut) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team wait-jobs: --format cannot be combined with --quiet or --json.")
				return exitErr(2)
			}
			...
			if nextStateChanged {
				...
				if err != nil {
					fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team wait-jobs: %s\n", strings.ReplaceAll(err.Error(), "--state", "--next-state"))
					return exitErr(2)
				}
			}
			...
			if err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team wait-jobs: %v\n", err)
				return exitErr(2)
			}
			if len(waitStatuses) == 0 && len(waitEvents) == 0 && !nextStateChanged && nextStepFilter == "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team wait-jobs: pass at least one non-empty --status, --event, --next-state, or --step.")
				return exitErr(2)
			}
			tmpl, err := parseJobFormat(format)
			if err != nil {
			...
			jobs, err := selectedTeamWaitJobs(teamDir, args[0])
			if err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team wait-jobs: %v\n", err)
				return exitErr(1)
			}
			jobs, err = filterTeamWaitJobs(jobs, jobFilters)
			if err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team wait-jobs: %v\n", err)
				return exitErr(2)
			}
			...
			if err != nil {
				if timeoutErr, ok := err.(*pipelineWaitTimeoutError); ok {
					if !quiet {
						fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team wait-jobs: timed out waiting for %s to reach %s: %s\n",
							args[0], jobWaitConditionList(waitStatuses, waitEvents, nextStateFilter, nextStateChanged, nextStepFilter), pipelineWaitPendingSummaryWithNext(timeoutErr.Pending, nextStateChanged || nextStepFilter != ""))
					}
					return exitErr(1)
				}
	...
	cmd.Flags().StringVar(&repo, "repo", cwd, repoFlagHelp)
	cmd.Flags().StringSliceVar(&jobFilters, "job", nil, "Only wait for these team-owned job ids. Can repeat or comma-separate.")
	...
	cmd.Flags().StringSliceVar(&events, "event", nil, "Last event to wait for, e.g. closed, adopted, pipeline_done, or pipeline_failed. Can repeat or comma-separate.")
	...
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit final team-owned jobs as JSON.")
...
func newTeamReadyCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := runTeamReady(cmd.OutOrStdout(), teamDir, args[0], stateFilter, opts, jsonOut, tmpl, commands, operatorCommandScopeFromCommand(cmd, repo, "repo")); err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team ready: %v\n", err)
				return exitErr(1)
			}
			return nil
...
func newTeamAdvanceCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if waitInterval < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team advance: --wait-interval must be >= 0.")
				return exitErr(2)
			}
			if waitTimeout < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team advance: --wait-timeout must be >= 0.")
				return exitErr(2)
			}
			...
			if dryRun && wait {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team advance: --wait cannot be combined with --dry-run.")
				return exitErr(2)
			}
			if !wait && (cmd.Flags().Changed("wait-status") || cmd.Flags().Changed("wait-event") || cmd.Flags().Changed("wait-next-state") || cmd.Flags().Changed("wait-step") || cmd.Flags().Changed("wait-timeout") || cmd.Flags().Changed("wait-interval") || failOnFailed) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team advance: wait-related flags require --wait.")
				return exitErr(2)
			}
			...
			if wait {
				results, err = waitForPipelineAdvanceResults(cmd, teamDir, results, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, "agent-team team advance")
				if err != nil {
					if err == context.Canceled {
						return nil
					}
...
func newTeamApproveCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if waitInterval < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team approve: --wait-interval must be >= 0.")
				return exitErr(2)
			}
			if waitTimeout < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team approve: --wait-timeout must be >= 0.")
				return exitErr(2)
			}
			...
			if dryRun && wait {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team approve: --wait cannot be combined with --dry-run.")
				return exitErr(2)
			}
			if !wait && (cmd.Flags().Changed("wait-status") || cmd.Flags().Changed("wait-event") || cmd.Flags().Changed("wait-next-state") || cmd.Flags().Changed("wait-step") || cmd.Flags().Changed("wait-timeout") || cmd.Flags().Changed("wait-interval") || failOnFailed) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team approve: wait-related flags require --wait.")
				return exitErr(2)
			}
	...
	cmd.Flags().StringSliceVar(&waitEvents, "wait-event", nil, "With --wait, last event to wait for, e.g. advance_dispatched, advance_queued, closed, or pipeline_done. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitNextState, "wait-next-state", nil, "With --wait, next-step state to wait for: ready, queued, running, blocked, failed, held, done, none, or all. Can repeat or comma-separate.")
	cmd.Flags().StringVar(&waitStep, "wait-step", "", "With --wait, pipeline step id that must be the current next step for every approved job.")
	cmd.Flags().DurationVar(&waitTimeout, "wait-timeout", 0, "Maximum time to wait with --wait (0 = no timeout).")
	cmd.Flags().DurationVar(&waitInterval, "wait-interval", 500*time.Millisecond, "Polling interval with --wait.")
...
func newTeamRetryCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if waitInterval < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team retry: --wait-interval must be >= 0.")
				return exitErr(2)
			}
			if waitTimeout < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team retry: --wait-timeout must be >= 0.")
				return exitErr(2)
			}
			if previewRoutes && (!dryRun || !dispatchNow) {
			...
			if dryRun && wait {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team retry: --wait cannot be combined with --dry-run.")
				return exitErr(2)
			}
			if !wait && (cmd.Flags().Changed("wait-status") || cmd.Flags().Changed("wait-event") || cmd.Flags().Changed("wait-next-state") || cmd.Flags().Changed("wait-step") || cmd.Flags().Changed("wait-timeout") || cmd.Flags().Changed("wait-interval") || failOnFailed) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team retry: wait-related flags require --wait.")
				return exitErr(2)
			}
	...
	cmd.Flags().StringSliceVar(&waitEvents, "wait-event", nil, "With --wait, last event to wait for, e.g. advance_dispatched, advance_queued, closed, or pipeline_done. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitNextState, "wait-next-state", nil, "With --wait, next-step state to wait for: ready, queued, running, blocked, failed, held, done, none, or all. Can repeat or comma-separate.")
	cmd.Flags().StringVar(&waitStep, "wait-step", "", "With --wait, pipeline step id that must be the current next step for every retried job.")
	cmd.Flags().DurationVar(&waitTimeout, "wait-timeout", 0, "Maximum time to wait with --wait (0 = no timeout).")
	cmd.Flags().DurationVar(&waitInterval, "wait-interval", 500*time.Millisecond, "Polling interval with --wait.")
...
func newTeamQueueShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			actions := teamQueueActionResolver(args[0])
			if commands {
...
func newTeamQueueQuarantineCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue quarantine: --limit must be >= 0.")
				return exitErr(2)
			}
			if summary && format != "" {
...
func newTeamQueueQuarantineRestoreCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue quarantine restore: --limit must be >= 0.")
				return exitErr(2)
			}
			teamDir, err := resolveTeamDir(cmd, repo)
			if err != nil {
			...
			if restoreAll {
				...
				if commands {
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueQuarantineRestoreResultsHaveDryRunAction(results, "would_restore"), queueApplyCommandOptions{
						BaseArgs:   []string{"agent-team", "team", "queue", "quarantine", "restore", args[0]},
						Repo:       repo,
						RepoSet:    cmd.Flags().Changed("repo"),
						All:        true,
						Force:      force,
			...
			if !filters.empty() || cmd.Flags().Changed("sort") || limit > 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue quarantine restore: filters require --all; --sort requires --all; --limit requires --all.")
				return exitErr(2)
			}
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), result.DryRun && result.Action == "would_restore", queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "team", "queue", "quarantine", "restore", args[0], result.Path},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
					Force:    force,
				})
	...
	cmd.Flags().StringVar(&sortBy, "sort", "path", "With --all, sort matching team-owned quarantined files before limiting: path, state, id, event, instance, job, queued, updated, modified, attempts, restorable, or size.")
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team queue quarantine restore apply command when the preview has actionable work.")
...
func newTeamQueueQuarantineDropCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue quarantine drop: --limit must be >= 0.")
				return exitErr(2)
			}
			if restorable && unrestorable {
			...
			if dropAll {
				...
				if commands {
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueQuarantineDropResultsHaveDryRunAction(results, "would_drop"), queueApplyCommandOptions{
						BaseArgs:     []string{"agent-team", "team", "queue", "quarantine", "drop", args[0]},
						Repo:         repo,
						RepoSet:      cmd.Flags().Changed("repo"),
						All:          true,
						State:        stateFilter,
			...
			if olderThan > 0 || restorable || unrestorable || !filters.empty() || cmd.Flags().Changed("sort") || limit > 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue quarantine drop: filters require --all; --sort requires --all; --limit requires --all.")
				return exitErr(2)
			}
			item, err := readTeamQueueQuarantineItem(teamDir, args[0], args[1])
			if err != nil {
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), result.DryRun && result.Action == "would_drop", queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "team", "queue", "quarantine", "drop", args[0], result.Path},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team queue quarantine drop apply command when the preview has actionable work.")
...
func newTeamQueueRetryCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if retryAll {
				...
				if commands {
					results, err := teamQueueRetryResults(teamDir, args[0], filters, sortMode, limit, true)
					if err != nil {
						return err
					}
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueRetryResultsHaveDryRunAction(results, "would_retry"), queueApplyCommandOptions{
						BaseArgs:   []string{"agent-team", "team", "queue", "retry", args[0]},
						Repo:       repo,
						RepoSet:    cmd.Flags().Changed("repo"),
						All:        true,
						State:      stateFilter,
				...
				return runTeamQueueRetryAll(cmd.OutOrStdout(), teamDir, args[0], filters, sortMode, limit, dryRun, jsonOut, tmpl)
			}
			if len(args) != 2 {
			...
			if stateFilter != "" || len(eventTypes) > 0 || len(jobs) > 0 || len(runtimes) > 0 || readyOnly || cmd.Flags().Changed("sort") || limit > 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue retry: --state, --event-type, --job, --runtime, --ready, --sort, and --limit require --all.")
				return exitErr(2)
			}
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), item != nil, queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "team", "queue", "retry", teamName, id},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team queue retry command when the preview has actionable work.")
...
func newTeamQueueDropCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dropAll {
				...
				if commands {
					results, err := teamQueueDropResults(teamDir, args[0], filters, sortMode, limit, true)
					if err != nil {
						return err
					}
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueDropResultsHaveDryRunAction(results, "would_drop"), queueApplyCommandOptions{
						BaseArgs:   []string{"agent-team", "team", "queue", "drop", args[0]},
						Repo:       repo,
						RepoSet:    cmd.Flags().Changed("repo"),
						All:        true,
						State:      stateFilter,
				...
				return runTeamQueueDropAll(cmd.OutOrStdout(), teamDir, args[0], filters, sortMode, limit, dryRun, jsonOut, tmpl)
			}
			if len(args) != 2 {
			...
			if stateFilter != "" || len(eventTypes) > 0 || len(jobs) > 0 || len(runtimes) > 0 || readyOnly || cmd.Flags().Changed("sort") || limit > 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team queue drop: --state, --event-type, --job, --runtime, --ready, --sort, and --limit require --all.")
				return exitErr(2)
			}
			teamName, id := args[0], args[1]
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), item != nil, queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "team", "queue", "drop", teamName, id},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team queue drop command when the preview has actionable work.")
	cmd.Flags().StringVar(&stateFilter, "state", "", "With --all, filter by queue state: pending or dead. Defaults to dead, or pending with --ready.")
	cmd.Flags().StringSliceVar(&eventTypes, "event-type", nil, "With --all, filter by event type; repeat or comma-separate values.")
	cmd.Flags().StringSliceVar(&jobs, "job", nil, "With --all, filter by job id or ticket; repeat or comma-separate values.")
	cmd.Flags().StringSliceVar(&runtimes, "runtime", nil, "With --all, filter by queued dispatch runtime: claude or codex. Can repeat or comma-separate.")
...
func newTeamQueuePruneCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				results, err := teamQueuePruneResults(teamDir, args[0], state, olderThan, filters, limit, time.Now().UTC(), true)
				if err != nil {
					return err
				}
				return renderQueueApplyCommand(cmd.OutOrStdout(), queuePruneResultsHaveDryRunAction(results), queueApplyCommandOptions{
					BaseArgs:     []string{"agent-team", "team", "queue", "prune", args[0]},
					Repo:         repo,
					RepoSet:      cmd.Flags().Changed("repo"),
					State:        stateFlag,
					StateSet:     cmd.Flags().Changed("state"),
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching team queue prune command when the preview has actionable work.")
...
func newTeamLogsCmd() *cobra.Command {
	...
	cmd.Flags().StringSliceVar(&jobs, "job", nil, "Only show logs for this team-owned job id or ticket. Can repeat or comma-separate.")
...
func newTeamSnapshotCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch {
			case commandsOnly:
				return renderSnapshotCommands(cmd.OutOrStdout(), snapshot, operatorCommandScopeFromCommand(cmd, repo, "repo"))
			case jsonOut || output == "-":
...
func newTeamPipelinesCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team pipelines: --limit must be >= 0.")
				return exitErr(2)
			}
			...
			if watch {
				ctx, stop := signal.NotifyContext(cmd.Context(), os.Interrupt)
				defer stop()
				return runTeamPipelinesWatch(ctx, cmd.OutOrStdout(), teamDir, args[0], sortMode, limit, jsonOut, tmpl, interval, !noClear && !jsonOut)
			}
			if err := runTeamPipelines(cmd.OutOrStdout(), teamDir, args[0], sortMode, limit, jsonOut, commands, tmpl, operatorCommandScopeFromCommand(cmd, repo, "repo")); err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team pipelines: %v\n", err)
				return exitErr(1)
			}
			return nil
...
func newTeamExplainCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := runTeamExplain(cmd.OutOrStdout(), teamDir, args[0], limit, stateFilter, step, sortMode, jsonOut, commands, tmpl, operatorCommandScopeFromCommand(cmd, repo, "repo")); err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "agent-team team explain: %v\n", err)
				return exitErr(1)
			}
			return nil
...
func newTeamTickCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				result.Tick.Advance, err = waitForPipelineAdvanceResults(cmd, teamDir, result.Tick.Advance, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, "agent-team team tick")
				if err != nil {
					if err == context.Canceled {
						return nil
					}
...
func newTeamDrainCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if waitInterval < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team drain: --wait-interval must be >= 0.")
				return exitErr(2)
			}
			if waitTimeout < 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team drain: --wait-timeout must be >= 0.")
				return exitErr(2)
			}
			if maxCycles <= 0 {
			...
			if wait && skipAdvance {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team drain: --wait requires pipeline advancement; remove --skip-advance.")
				return exitErr(2)
			}
			if !wait && (cmd.Flags().Changed("wait-status") || cmd.Flags().Changed("wait-event") || cmd.Flags().Changed("wait-next-state") || cmd.Flags().Changed("wait-step") || cmd.Flags().Changed("wait-timeout") || cmd.Flags().Changed("wait-interval") || failOnFailed) {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team drain: wait-related flags require --wait.")
				return exitErr(2)
			}
			tmpl, err := parseTeamTickFormat(format)
			if err != nil {
			...
			if wait {
				if err := waitForTeamTickUntilIdleResult(cmd, teamDir, result, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, "agent-team team drain"); err != nil {
					if err == context.Canceled {
						return nil
					}
					return err
			...
			if err := renderTeamTickUntilIdleResult(cmd.OutOrStdout(), result, jsonOut, tmpl); err != nil {
				return err
			}
			if failOnFailed && teamTickUntilIdleResultHasFailed(result) {
				return exitErr(1)
			}
			return nil
		},
	...
	cmd.Flags().BoolVar(&wait, "wait", false, "After team drain reaches idle, wait for jobs advanced during team drain cycles to reach a lifecycle status, event, or next-step state.")
	cmd.Flags().StringSliceVar(&waitStatuses, "wait-status", nil, "With --wait, status to wait for: queued, running, blocked, done, failed, or terminal. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitEvents, "wait-event", nil, "With --wait, last event to wait for, e.g. advance_dispatched, advance_queued, closed, or pipeline_done. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitNextState, "wait-next-state", nil, "With --wait, next-step state to wait for: ready, queued, running, blocked, failed, held, done, none, or all. Can repeat or comma-separate.")
	...
	cmd.Flags().DurationVar(&waitTimeout, "wait-timeout", 0, "Maximum time to wait with --wait (0 = no timeout).")
	cmd.Flags().DurationVar(&waitInterval, "wait-interval", 500*time.Millisecond, "Polling interval with --wait.")
	cmd.Flags().BoolVar(&failOnFailed, "fail-on-failed", false, "With --wait, exit 1 if any team drain-advanced job resolves to failed.")
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit machine-readable JSON.")
...
func newTeamRepairCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.TrimSpace(timeoutMessageFile) != "" && !timeoutPipelines && !timeoutJobs {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team repair: --timeout-message-file requires --timeout-pipelines or --timeout-jobs.")
				return exitErr(2)
			}
			...
			if strings.TrimSpace(retryMessageFile) != "" && !retryPipelines {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team repair: --retry-message-file requires --retry-pipelines.")
				return exitErr(2)
			}
			...
			if wait {
				if err := waitForTeamRepairResult(cmd, teamDir, args[0], result, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, includeJobs); err != nil {
					if err == context.Canceled {
						return nil
					}
					return err
	...
	cmd.Flags().StringVar(&timeoutMessageFile, "timeout-message-file", "", "Read team timeout repair audit message from a file, or '-' for stdin.")
	...
	cmd.Flags().StringVar(&retryMessageFile, "retry-message-file", "", "Read team retry repair audit message from a file, or '-' for stdin.")
	...
	cmd.Flags().IntVar(&maxCycles, "max-cycles", 20, "With --until-idle, stop after this many cycles if work keeps appearing.")
	cmd.Flags().BoolVar(&wait, "wait", false, "After team repair dispatches retried or ready steps, wait for those jobs to reach a lifecycle status, event, or next-step state.")
	cmd.Flags().StringSliceVar(&waitStatuses, "wait-status", nil, "With --wait, status to wait for: queued, running, blocked, done, failed, or terminal. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitEvents, "wait-event", nil, "With --wait, last event to wait for, e.g. advance_dispatched, advance_queued, closed, or pipeline_done. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitNextState, "wait-next-state", nil, "With --wait, next-step state to wait for: ready, queued, running, blocked, failed, held, done, none, or all. Can repeat or comma-separate.")
...
func newTeamStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				return renderTeamStatusCommands(cmd.OutOrStdout(), snapshot, operatorCommandScopeFromCommand(cmd, repo, "repo"))
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.TrimSpace(eventSortBy) != "" && cmd.Flags().Changed("events-sort") && eventTail == 0 {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team monitor: --events-sort requires --events.")
				return exitErr(2)
			}
			...
			eventSortMode, err := parseEventSort(eventSortBy)
			if err != nil {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team monitor: --events-sort must be oldest or newest.")
				return exitErr(2)
			}
...
func collectTeamJobs(teamDir, name string, filters jobListFilters) ([]*job.Job, error) {
	top, team, err := loadTopologyTeam(teamDir, name)
	if err != nil {
		return nil, err
	}
...
func selectedTeamWaitJobs(teamDir, name string) ([]*job.Job, error) {
	top, team, err := loadTopologyTeam(teamDir, name)
	if err != nil {
		return nil, err
	}
	...
	return teamJobs(top, team, jobs), nil
}

func filterTeamWaitJobs(jobs []*job.Job, filters []string) ([]*job.Job, error) {
	...
	if len(missing) > 0 {
		...
		return nil, fmt.Errorf("job(s) not owned by team: %s", strings.Join(missing, ", "))
	}
	return out, nil
}

func runTeamJobs(w io.Writer, teamDir, name string, filters jobListFilters, summary bool, jsonOut bool, tmpl *template.Template) error {
	jobs, err := collectTeamJobs(teamDir, name, filters)
	if err != nil {
		return err
	}
	if summary {
...
func runTeamJobsWatch(ctx context.Context, w io.Writer, teamDir, name string, filters jobListFilters, summary bool, jsonOut bool, tmpl *template.Template, interval time.Duration, clear bool) error {
	if interval <= 0 {
	...
	for {
		...
		if err := runTeamJobs(w, teamDir, name, filters, summary, jsonOut, tmpl); err != nil {
			return err
		}
...
func scopeTeamTriageItemActions(teamName string, item jobTriageItem) []string {
	...
	return replaceOrAppendScopedTriageAction(actions, jobAction, teamAction)
}
...
func replaceOrAppendScopedTriageAction(actions []string, oldAction, newAction string) []string {
...
func runTeamReady(w io.Writer, teamDir, name string, states map[string]bool, opts jobReadyOptions, jsonOut bool, tmpl *template.Template, commands bool, scope operatorCommandScope) error {
	rows, err := collectTeamReadyRows(teamDir, name, states)
	if err != nil {
		return err
	}
	rows = prepareJobReadyRows(rows, opts)
	return renderTeamReadyRows(w, rows, jsonOut, tmpl, commands, scope)
}
...
func runTeamReadyWatch(ctx context.Context, w io.Writer, teamDir, name string, states map[string]bool, opts jobReadyOptions, jsonOut bool, tmpl *template.Template, interval time.Duration, clear bool) error {
	...
	for {
		...
		if err := runTeamReady(w, teamDir, name, states, opts, jsonOut, tmpl, false, operatorCommandScope{}); err != nil {
			return err
		}
		if !waitForWatchTick(ctx, ticker.C) {
			return nil
...
func renderTeamReadyRows(w io.Writer, rows []jobReadyRow, jsonOut bool, tmpl *template.Template, commands bool, scope operatorCommandScope) error {
	if jsonOut {
...
func runTeamQueueDropAll(w io.Writer, teamDir, name string, filters queueListFilters, sortMode string, limit int, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := teamQueueDropResults(teamDir, name, filters, sortMode, limit, dryRun)
	if err != nil {
		return err
	}
...
func teamQueueDropResults(teamDir, name string, filters queueListFilters, sortMode string, limit int, dryRun bool) ([]queueDropResult, error) {
...
func runTeamQueuePrune(w io.Writer, teamDir, name, state string, olderThan time.Duration, filters queueListFilters, limit int, now time.Time, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := teamQueuePruneResults(teamDir, name, state, olderThan, filters, limit, now, dryRun)
	if err != nil {
		return err
	}
...
func teamQueuePruneResults(teamDir, name, state string, olderThan time.Duration, filters queueListFilters, limit int, now time.Time, dryRun bool) ([]queuePruneResult, error) {
...
func runTeamQueueRetryAll(w io.Writer, teamDir, name string, filters queueListFilters, sortMode string, limit int, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := teamQueueRetryResults(teamDir, name, filters, sortMode, limit, dryRun)
	if err != nil {
		return err
	}
...
func teamQueueRetryResults(teamDir, name string, filters queueListFilters, sortMode string, limit int, dryRun bool) ([]queueRetryResult, error) {
	matches, err := collectTeamQueueItems(teamDir, name, filters, time.Now().UTC())
	if err != nil {
		return nil, err
	}
...
func runTeamPipelines(w io.Writer, teamDir, name, sortMode string, limit int, jsonOut, commands bool, tmpl *template.Template, scope operatorCommandScope) error {
...
func runTeamPipelinesWatch(ctx context.Context, w io.Writer, teamDir, name, sortMode string, limit int, jsonOut bool, tmpl *template.Template, interval time.Duration, clear bool) error {
	if interval <= 0 {
		interval = 2 * time.Second
	}
	ticker := time.NewTicker(interval)
	...
	for {
		...
		if err := runTeamPipelines(w, teamDir, name, sortMode, limit, jsonOut, false, tmpl, operatorCommandScope{}); err != nil {
			return err
		}
		if !waitForWatchTick(ctx, ticker.C) {
			return nil
...
func runTeamExplain(w io.Writer, teamDir, name string, limit int, stateFilter map[string]bool, stepFilter string, sortMode string, jsonOut, commands bool, tmpl *template.Template, scope operatorCommandScope) error {
...
func runTeamExplainWatch(ctx context.Context, w io.Writer, teamDir, name string, limit int, stateFilter map[string]bool, stepFilter string, sortMode string, jsonOut bool, tmpl *template.Template, interval time.Duration, clear bool) error {
	...
	for {
		...
		if err := runTeamExplain(w, teamDir, name, limit, stateFilter, stepFilter, sortMode, jsonOut, false, tmpl, operatorCommandScope{}); err != nil {
			return err
		}
		if !waitForWatchTick(ctx, ticker.C) {
			return nil
...
func waitForTeamTickUntilIdleResult(cmd *cobra.Command, teamDir string, result *teamTickUntilIdleResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration, prefix string) error {
	if result == nil {
		return nil
	}
	for _, cycle := range result.Cycles {
		...
		if err := waitForTickResultAdvanceRows(cmd, teamDir, &cycle.Tick, statuses, events, nextStates, nextStateSet, step, timeout, interval, prefix); err != nil {
			return err
		}
	}
	return nil
...
func teamTickUntilIdleResultHasFailed(result *teamTickUntilIdleResult) bool {
	if result == nil {
		return false
	}
	for _, cycle := range result.Cycles {
...
func runTeamRepair(cmd *cobra.Command, repo, teamDir, name string, opts teamRepairOptions) (*teamRepairResult, error) {
	...
	if opts.SkipQueue {
		...
		retries, err := teamQueueRetryResults(teamDir, name, filters, "state", opts.Limit, opts.DryRun)
		if err != nil {
			return nil, err
		}
...
func waitForTeamRepairResult(cmd *cobra.Command, teamDir, name string, result *teamRepairResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration, includeJobs bool) error {
	if result == nil {
		return nil
	}
	var err error
	result.PipelineRetry.Results, err = waitForPipelineRetryResults(cmd, teamDir, result.PipelineRetry.Results, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team team repair")
	if err != nil {
		return err
	}
	if result.Tick.Result != nil {
		if err := waitForTickResultAdvanceRows(cmd, teamDir, &result.Tick.Result.Tick, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team team repair"); err != nil {
			return err
		}
	}
	if result.Tick.UntilIdle != nil {
		for _, cycle := range result.Tick.UntilIdle.Cycles {
			...
			if err := waitForTickResultAdvanceRows(cmd, teamDir, &cycle.Tick, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team team repair"); err != nil {
				return err
			}
		}
	}
...
func renderTeamStatusCommands(w io.Writer, snapshot *teamStatusSnapshot, scope operatorCommandScope) error {
	if snapshot == nil {
		return nil
	}
