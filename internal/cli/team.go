		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				...
				if len(waitStatusesSet) == 0 && len(waitEventsSet) == 0 {
					fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team advance: pass at least one non-empty --wait-status or --wait-event.")
					return exitErr(2)
				}
			}
			teamDir, err := resolveTeamDir(cmd, repo)
			...
			if wait {
				results, err = waitForPipelineAdvanceResults(cmd, teamDir, results, waitStatusesSet, waitEventsSet, waitTimeout, waitInterval, "agent-team team advance")
				if err != nil {
					if err == context.Canceled {
						return nil
					}
