...
func waitForTeamRepairResult(cmd *cobra.Command, teamDir, name string, result *teamRepairResult, statuses map[job.Status]bool, events map[string]bool, timeout, interval time.Duration, includeJobs bool) error {
	...
	var err error
	result.PipelineRetry.Results, err = waitForPipelineRetryResults(cmd, teamDir, result.PipelineRetry.Results, statuses, events, nil, false, "", timeout, interval, "agent-team team repair")
	if err != nil {
		return err
	}
