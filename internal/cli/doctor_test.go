...
func TestDoctorFailsOnSecretKeyInConfig(t *testing.T) {
	tmp := t.TempDir()
	initInto(t, tmp)
...
`), 0o644); err != nil {
	...
	if result.OK || !containsDoctorMessage(result.Problems, "[github].api_key") {
		t.Fatalf("doctor result = %+v, want [github].api_key problem", result)
	}
	if strings.Contains(out.String(), "do-not-print-this-secret") || strings.Contains(errOut.String(), "do-not-print-this-secret") {
		t.Fatalf("doctor output leaked the secret value\nstdout=%s\nstderr=%s", out.String(), errOut.String())
	}
}

func TestDoctorFailsOnSecretLiteralInConfigValue(t *testing.T) {
	tmp := t.TempDir()
	initInto(t, tmp)
...
[notifications]
endpoint = "https://discord.com/api/webhooks/1234567890/abcdefghijklmnopqrstuvwxyz_123456"
`), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if result.OK || !containsDoctorMessage(result.Problems, "[notifications].endpoint") || !containsDoctorMessage(result.Problems, "Discord webhook URL literal") {
		t.Fatalf("doctor result = %+v, want webhook literal problem", result)
	}
	if strings.Contains(out.String(), "abcdefghijklmnopqrstuvwxyz_123456") || strings.Contains(errOut.String(), "abcdefghijklmnopqrstuvwxyz_123456") {
		t.Fatalf("doctor output leaked the secret value\nstdout=%s\nstderr=%s", out.String(), errOut.String())
	}
}
...
[otel.headers]
authorization = "${OTEL_AUTHORIZATION_HEADER}"
`), 0o644); err != nil {
		t.Fatal(err)
	}
