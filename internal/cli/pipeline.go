...
func newPipelineJobEventsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if summary && follow {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team pipeline job-events: --summary cannot be combined with --follow.")
				return exitErr(2)
			}
			if interval < 0 {
			...
			loadJobs := func() ([]*job.Job, error) {
				return collectPipelineEventJobs(teamDir, pipelineName)
			}
			if follow {
				ctx, stop := signal.NotifyContext(cmd.Context(), os.Interrupt)
				defer stop()
	...
	cmd.Flags().BoolVarP(&follow, "follow", "f", false, "Poll and print new pipeline job events until interrupted.")
...
func newPipelineQueueRetryCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if retryAll {
				...
				if commands {
					results, err := pipelineQueueRetryResults(teamDir, args[0], filters, sortMode, limit, true)
					if err != nil {
						return err
					}
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueRetryResultsHaveDryRunAction(results, "would_retry"), queueApplyCommandOptions{
						BaseArgs:   []string{"agent-team", "pipeline", "queue", "retry", args[0]},
						Repo:       repo,
						RepoSet:    cmd.Flags().Changed("repo"),
						All:        true,
						State:      stateFilter,
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), item != nil, queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "pipeline", "queue", "retry", args[0], args[1]},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline queue retry command when the preview has actionable work.")
...
func newPipelineQueueDropCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dropAll {
				...
				if commands {
					results, err := pipelineQueueDropResults(teamDir, args[0], filters, sortMode, limit, true)
					if err != nil {
						return err
					}
					return renderQueueApplyCommand(cmd.OutOrStdout(), queueDropResultsHaveDryRunAction(results, "would_drop"), queueApplyCommandOptions{
						BaseArgs:   []string{"agent-team", "pipeline", "queue", "drop", args[0]},
						Repo:       repo,
						RepoSet:    cmd.Flags().Changed("repo"),
						All:        true,
						State:      stateFilter,
			...
			if commands {
				return renderQueueApplyCommand(cmd.OutOrStdout(), item != nil, queueApplyCommandOptions{
					BaseArgs: []string{"agent-team", "pipeline", "queue", "drop", args[0], args[1]},
					Repo:     repo,
					RepoSet:  cmd.Flags().Changed("repo"),
				})
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline queue drop command when the preview has actionable work.")
	cmd.Flags().StringVar(&stateFilter, "state", "", "With --all, filter by queue state: pending or dead. Defaults to dead, or pending with --ready.")
	cmd.Flags().StringSliceVar(&eventTypes, "event-type", nil, "With --all, filter by event type; repeat or comma-separate values.")
	cmd.Flags().StringSliceVar(&jobs, "job", nil, "With --all, filter by job id or ticket; repeat or comma-separate values.")
	cmd.Flags().StringSliceVar(&runtimes, "runtime", nil, "With --all, filter by queued dispatch runtime: claude or codex. Can repeat or comma-separate.")
...
func newPipelineQueuePruneCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				results, err := pipelineQueuePruneResults(teamDir, args[0], state, olderThan, filters, limit, time.Now().UTC(), true)
				if err != nil {
					return err
				}
				return renderQueueApplyCommand(cmd.OutOrStdout(), queuePruneResultsHaveDryRunAction(results), queueApplyCommandOptions{
					BaseArgs:     []string{"agent-team", "pipeline", "queue", "prune", args[0]},
					Repo:         repo,
					RepoSet:      cmd.Flags().Changed("repo"),
					State:        stateFlag,
					StateSet:     cmd.Flags().Changed("state"),
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline queue prune command when the preview has actionable work.")
...
func newPipelineAdvanceCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				results, err = waitForPipelineAdvanceResults(cmd, teamDir, results, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, "agent-team pipeline advance")
				if err != nil {
					if err == context.Canceled {
						return nil
					}
...
func newPipelineRejectCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				baseArgs := []string{"agent-team", "pipeline", "reject"}
				if !all {
					baseArgs = append(baseArgs, pipelineName)
				}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching reject apply command when the preview has actionable work.")
...
func newPipelineSkipCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				baseArgs := []string{"agent-team", "pipeline", "skip"}
				if !all {
					baseArgs = append(baseArgs, pipelineName)
				}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching skip apply command when the preview has actionable work.")
...
func newPipelineCancelCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				baseArgs := []string{"agent-team", "pipeline", "cancel"}
				if !all {
					baseArgs = append(baseArgs, pipelineName)
				}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching cancel apply command when the preview has actionable work.")
...
func newPipelineSendCmd() *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				...
				return renderScopedSendApplyCommand(cmd.OutOrStdout(), len(targets) > 0, scopedSendApplyCommandOptions{
					BaseArgs:       []string{"agent-team", "pipeline", "send", pipelineName},
					Repo:           repo,
					RepoSet:        cmd.Flags().Changed("repo"),
					From:           from,
					FromSet:        cmd.Flags().Changed("from"),
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline send apply command when the preview has actionable recipients.")
	cmd.Flags().BoolVar(&jsonOut, "json", false, "Emit machine-readable JSON.")
...
func newPipelineEventsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if follow && sortMode == "newest" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team pipeline events: --sort newest cannot be combined with --follow.")
				return exitErr(2)
			}
...
func newPipelineRepairCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				if err := waitForPipelineRepairResult(cmd, teamDir, result, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval); err != nil {
					if err == context.Canceled {
						return nil
					}
					return err
			...
			if commands {
				return renderPipelineRepairCommands(cmd.OutOrStdout(), result, repairApplyCommandOptions{
					BaseArgs:              []string{"agent-team", "pipeline", "repair", args[0]},
					ScopeFlag:             "--repo",
					Scope:                 repo,
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline repair apply command when the preview has actionable work.")
...
func newPipelineTickCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				result.Tick.Advance, err = waitForPipelineAdvanceResults(cmd, teamDir, result.Tick.Advance, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval, "agent-team pipeline tick")
				if err != nil {
					if err == context.Canceled {
						return nil
					}
...
func newPipelineDrainCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wait {
				if err := waitForPipelineDrainResult(cmd, teamDir, result, waitFilters.statuses, waitFilters.events, waitFilters.nextStates, waitFilters.nextStateSet, waitFilters.step, waitTimeout, waitInterval); err != nil {
					if err == context.Canceled {
						return nil
					}
					return err
	...
	cmd.Flags().BoolVar(&wait, "wait", false, "After pipeline drain reaches idle, wait for jobs advanced during drain cycles to reach a lifecycle status, event, or next-step state.")
	cmd.Flags().StringSliceVar(&waitStatuses, "wait-status", nil, "With --wait, status to wait for: queued, running, blocked, done, failed, or terminal. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitEvents, "wait-event", nil, "With --wait, last event to wait for, e.g. advance_dispatched, advance_queued, closed, or pipeline_done. Can repeat or comma-separate.")
	cmd.Flags().StringSliceVar(&waitNextState, "wait-next-state", nil, "With --wait, next-step state to wait for: ready, queued, running, blocked, failed, held, done, none, or all. Can repeat or comma-separate.")
...
func newPipelineHoldCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				baseArgs := []string{"agent-team", "pipeline", "hold"}
				if !all {
					baseArgs = append(baseArgs, pipelineName)
				}
...
func newPipelineReleaseCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands {
				baseArgs := []string{"agent-team", "pipeline", "release"}
				if !all {
					baseArgs = append(baseArgs, pipelineName)
				}
...
func newPipelineRunCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "With --dry-run, print the matching pipeline run apply command. agent-team follow-ups preserve the selected repo scope.")
...
func runPipelineQueueDropAll(w io.Writer, teamDir, pipeline string, filters queueListFilters, sortMode string, limit int, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := pipelineQueueDropResults(teamDir, pipeline, filters, sortMode, limit, dryRun)
	if err != nil {
		return err
	}
...
func pipelineQueueDropResults(teamDir, pipeline string, filters queueListFilters, sortMode string, limit int, dryRun bool) ([]queueDropResult, error) {
	matches, err := collectPipelineQueueItems(teamDir, pipeline, filters, time.Now().UTC())
	if err != nil {
		return nil, err
	}
...
func runPipelineQueuePrune(w io.Writer, teamDir, pipeline, state string, olderThan time.Duration, filters queueListFilters, limit int, now time.Time, dryRun, jsonOut bool, tmpl *template.Template) error {
	results, err := pipelineQueuePruneResults(teamDir, pipeline, state, olderThan, filters, limit, now, dryRun)
	if err != nil {
		return err
	}
...
func pipelineQueuePruneResults(teamDir, pipeline, state string, olderThan time.Duration, filters queueListFilters, limit int, now time.Time, dryRun bool) ([]queuePruneResult, error) {
	items, err := collectPipelineQueueItems(teamDir, pipeline, filters, now)
	if err != nil {
		return nil, err
	}
...
func parseJobCommandWaitFilters(cmd *cobra.Command, waitStatuses, waitEvents, waitNextStates []string, waitStep string) (jobWaitFilters, error) {
	...
	nextStateChanged := cmd.Flags().Changed("wait-next-state")
	if nextStateChanged {
		nextStates, err := parseJobNextStateFilter(waitNextStates, false)
		if err != nil {
			return filters, fmt.Errorf("%s", strings.ReplaceAll(err.Error(), "--state", "--wait-next-state"))
		}
		filters.nextStates = nextStates
		filters.nextStateSet = true
	}
	defaultStatuses := !cmd.Flags().Changed("wait-status") && len(filters.events) == 0 && !filters.nextStateSet && filters.step == ""
	statuses, err := parseJobWaitStatuses(waitStatuses, defaultStatuses)
	if err != nil {
		return filters, err
	}
	filters.statuses = statuses
	if len(filters.statuses) == 0 && len(filters.events) == 0 && !filters.nextStateSet && filters.step == "" {
		return filters, fmt.Errorf("pass at least one non-empty --wait-status, --wait-event, --wait-next-state, or --wait-step")
	}
	return filters, nil
}

func waitForPipelineAdvanceResults(cmd *cobra.Command, teamDir string, results []pipelineAdvanceResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration, prefix string) ([]pipelineAdvanceResult, error) {
	...
	defer cancel()
	waited, err := runPipelineWait(ctx, teamDir, jobs, statuses, events, nextStates, nextStateSet, step, interval)
	if err != nil {
		if timeoutErr, ok := err.(*pipelineWaitTimeoutError); ok {
			fmt.Fprintf(cmd.ErrOrStderr(), "%s: timed out waiting for advanced jobs to reach %s (pending=%s).\n",
				prefix, jobWaitConditionList(statuses, events, nextStates, nextStateSet, step), pipelineWaitPendingSummaryWithNext(timeoutErr.Pending, nextStateSet || strings.TrimSpace(step) != ""))
			return nil, exitErr(1)
		}
		return nil, err
	}
...
func waitForPipelineApproveResults(cmd *cobra.Command, teamDir string, results []pipelineApproveResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration, prefix string) ([]pipelineApproveResult, error) {
	ids := make([]string, 0, len(results))
	seen := map[string]bool{}
	for _, result := range results {
		id := strings.TrimSpace(result.JobID)
...
func waitForPipelineRetryResults(cmd *cobra.Command, teamDir string, results []pipelineRetryResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration, prefix string) ([]pipelineRetryResult, error) {
	ids := make([]string, 0, len(results))
	seen := map[string]bool{}
	for _, result := range results {
		id := strings.TrimSpace(result.JobID)
...
func waitForPipelineRepairResult(cmd *cobra.Command, teamDir string, result *pipelineRepairResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration) error {
	if result == nil {
		return nil
	}
	var err error
	result.PipelineRetry.Results, err = waitForPipelineRetryResults(cmd, teamDir, result.PipelineRetry.Results, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team pipeline repair")
	if err != nil {
		return err
	}
	result.Advance.Results, err = waitForPipelineAdvanceResults(cmd, teamDir, result.Advance.Results, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team pipeline repair")
	if err != nil {
		return err
	}
	if !result.DryRun {
...
func waitForPipelineDrainResult(cmd *cobra.Command, teamDir string, result *pipelineDrainResult, statuses map[job.Status]bool, events map[string]bool, nextStates map[string]bool, nextStateSet bool, step string, timeout, interval time.Duration) error {
	if result == nil {
		return nil
	}
	for _, cycle := range result.Cycles {
		...
		if err := waitForTickResultAdvanceRows(cmd, teamDir, &cycle.Tick, statuses, events, nextStates, nextStateSet, step, timeout, interval, "agent-team pipeline drain"); err != nil {
			return err
		}
	}
	return nil
...
func pipelineExplainCommandActions(rows []pipelineExplainRow) []string {
	var actions []string
	for _, row := range rows {
		actions = append(actions, row.Actions...)
		for _, explained := range row.Jobs {
			actions = append(actions, jobExplainCommandActions(explained)...)
		}
	}
	return commandActionsOnly(actions)
}
...
type pipelineApplyCommandOptions struct {
	BaseArgs       []string
	Repo           string
	RepoSet        bool
	All            bool
...
func renderPipelineApplyCommand(w io.Writer, hasAction bool, opts pipelineApplyCommandOptions) error {
	...
	_, err := fmt.Fprintln(w, strings.Join(shellQuoteArgs(pipelineApplyCommandArgs(opts)), " "))
	return err
}

func pipelineApplyCommandArgs(opts pipelineApplyCommandOptions) []string {
	args := append([]string{}, opts.BaseArgs...)
	if opts.RepoSet && strings.TrimSpace(opts.Repo) != "" {
		args = append(args, "--repo", opts.Repo)
	}
...
func pipelineSkipResultsHaveDryRunAction(results []pipelineSkipResult, action string) bool {
	for _, result := range results {
		if result.DryRun && strings.TrimSpace(result.Action) == action {
			return true
		}
