...
func newTeamOutboxQuarantineCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if commands && jsonOut {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team outbox quarantine: --commands cannot be combined with --json.")
				return exitErr(2)
			}
			if commands && format != "" {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team outbox quarantine: --commands cannot be combined with --format.")
				return exitErr(2)
			}
			if commands && summary {
				fmt.Fprintln(cmd.ErrOrStderr(), "agent-team team outbox quarantine: --commands cannot be combined with --summary.")
				return exitErr(2)
			}
			...
			if commands {
				return renderOutboxQuarantineListCommands(cmd.OutOrStdout(), items, scopedOutboxQuarantineActionResolver("", "", args[0]), operatorCommandScopeFromCommand(cmd, repo, "repo"))
			}
	...
	cmd.Flags().BoolVar(&commands, "commands", false, "Print recommended commands from the visible team-owned quarantined outbox files, one per line. agent-team follow-ups preserve the selected repo scope.")
