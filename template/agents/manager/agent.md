...
5. **Invoke the worker dispatch path** once the work item is concrete: use the `assign-worker` skill for the normal manager-to-worker handoff, or the daemon's `agent-team job create ... --dispatch --workspace worktree` path for ticketless jobs that don't already have a durable job. Keep criteria as observable outcomes, not implementation plans. Don't implement the ticket yourself.
