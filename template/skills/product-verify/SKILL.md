---
name: product-verify
description: "Run the scheduled product-verifier loop: headless synthetic-user checks of daemon UI data against CLI ground truth, plus capped feedback filing for correctness bugs, UX friction, and product ideas."
---
...
v1 is headless and has no browser dependency. Do not use Playwright or a real
browser in this loop. v2 browser automation is a follow-up once a
browser-capable runtime is available.

Primary objective:

1. Fetch the daemon UI data endpoints with the operator bearer token:
   `/v1/instances`, `/v1/jobs`, and `/v1/topology`.
2. Read equivalent state through the CLI.
3. Diff the two views. Any mismatch is an objective product correctness bug.
4. File each capped mismatch with `agent-team feedback submit --category bug`.
...
- The only intended write is `agent-team feedback submit`.
...
2. Correctness: did the mechanical UI-vs-CLI diff pass?
3. Token flow: does the operator-token requirement make sense from the served
   API behavior and docs?
...
A run is complete when the mechanical diff either passed or skipped for lack of
HTTP configuration, capped bug findings were submitted, subjective friction or
idea findings were submitted when present, and you have recorded a short summary
to your manager state or inbox.
