...
Pipeline state is recorded in the job file, so `job show`, `job explain`, `job ready`, `job advance`, `pipeline status`, and team-scoped commands can reason about the next step. Gates such as `manual` and `pr` are also stored on job steps, making waiting reasons visible in the same commands; manual gates can be approved with `pipeline approve`, `team approve`, or the lower-level `job step`.
