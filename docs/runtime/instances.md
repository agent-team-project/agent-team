...
`inspect` combines runtime metadata, daemon-known topology, status file data, and state paths. The runtime block includes both the selected runtime kind and binary/wrapper path when daemon metadata records it; `ps --json` and `stats --json` expose the same `runtime` and `runtime_binary` fields for scripts. Use `ps --runtime codex`, `stats --runtime codex`, or `logs --runtime codex` to narrow mixed-runtime views.
