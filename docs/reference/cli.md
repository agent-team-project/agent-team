...
| `agent-team runtime resume-plan` | Show resume, attach, and log fallback commands from daemon metadata |
...
| `agent-team team runtime resume-plan <team>` | Team-scoped runtime recovery commands |
...
| `agent-team team repair <team>` | Scoped repair loop, including stale-work timeout with `--timeout-jobs` |
...
| `agent-team repair` | Start/reconcile/timeout/retry/tick recovery loop |
