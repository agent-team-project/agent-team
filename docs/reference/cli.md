...
| `agent-team pipeline queue [<pipeline>]` | List or summarize active/quarantined queue items owned by one or all pipelines; subcommands inspect, retry, drop, prune, or recover items owned by one pipeline; `queue show` and `queue quarantine show --commands` preserve an explicit `--repo` selector |
...
| `agent-team team monitor <team>` | Scoped dashboard with team-owned runtime, queue, and outbox recovery signals; add `--last-message` when Codex log fallback hints should prefer clean final responses, or `--commands` for one scoped command per line from visible recovery sections |
| `agent-team team watch <team>` | Continuous scoped dashboard with team-owned runtime, queue, and outbox recovery signals; add `--last-message` for the same Codex final-message preference |
...
| `agent-team monitor` | Operator dashboard with health, job/queue/outbox recovery, inbox, instances, and resources; add `--last-message` when Codex log fallback hints should prefer clean final responses, or `--commands` for one command per line from visible recovery sections |
| `agent-team watch` | Continuous monitor with health, job/queue/outbox recovery, inbox, instances, and resources; add `--last-message` for the same Codex final-message preference |
