...
| `agent-team resume-plan` | Show resume, attach, and log fallback commands from daemon metadata; filter by `--step`, `--action`, `--runtime-stale`, `--unhealthy`, `--managed`, `--can-managed`, or `--direct`, prefer clean Codex sidecars with `--last-message`, sort/limit large recovery lists, print repo-scoped commands with `--commands`, or summarize with `--summary`; `runtime resume-plan` remains available |
...
| `agent-team job resume-plan <job-id>` | Show runtime resume, attach, and log fallback commands for one job; add `--step` for one pipeline stage, capability filters such as `--can-managed` or `--direct`, `--last-message` for clean Codex sidecars, `--commands` for repo-scoped command lines, or `--sort`/`--limit` for multi-runtime jobs |
...
| `agent-team pipeline resume-plan [<pipeline>]` | Pipeline-owned runtime recovery commands across all workflows by default; filter by `--step`, `--action`, `--runtime-stale`, `--unhealthy`, `--managed`, `--can-managed`, or `--direct`, prefer clean Codex sidecars with `--last-message`, sort/limit large recovery lists, print repo-scoped commands with `--commands`, or summarize with `--summary` |
...
| `agent-team team resume-plan <team>` | Team-scoped runtime recovery commands; filter by `--step`, `--action`, `--runtime-stale`, `--unhealthy`, `--managed`, `--can-managed`, or `--direct`, prefer clean Codex sidecars with `--last-message`, sort/limit large recovery lists, print repo-scoped commands with `--commands`, or summarize with `--summary`; `team runtime resume-plan` remains available |
