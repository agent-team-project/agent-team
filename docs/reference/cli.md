...
| `agent-team snapshot diff <before.json> <after.json>` | Compare saved global, team, pipeline, or job diagnostic artifacts, including provenance, git/runtime context, health, plan, triage, next-action hints, job state, inboxes, queue, schedules, intake, events, pipeline state, bounded detail rows, and formatted counters for scripts |
