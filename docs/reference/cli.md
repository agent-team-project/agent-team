...
| `agent-team snapshot diff <before.json> <after.json>` | Compare saved global, team, pipeline, or job diagnostic artifacts, including provenance, git/runtime context, health, plan, next-action hints, job state, inboxes, queue, schedules, intake, events, and pipeline state |
