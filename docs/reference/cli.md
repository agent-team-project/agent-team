...
| `agent-team job explain <job-id>` | Explain or watch pipeline step readiness, blockers, gates, and next actions; add `--step` to focus one stage |
...
| `agent-team job reconcile github` | Reconcile GitHub PR payloads into jobs; add `--advance --wait` for bounded PR-gate handoff |
...
| `agent-team repair` | Start/reconcile/timeout/retry/tick recovery loop; timeout repair accepts filters, and failed-step retry accepts pipeline/step filters, `--retry-force`, runtime overrides, and `--wait` bounded handoff |
