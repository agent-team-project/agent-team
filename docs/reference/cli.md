...
| `agent-team job ready` | List next pipeline steps; filter by `--step` and sort by `--sort` |
...
| `agent-team pipeline ready` | List ready steps; filter by `--step` and sort by `--sort` |
...
| `agent-team team ready <team>` | Scoped ready pipeline steps; filter by `--step` and sort by `--sort` |
