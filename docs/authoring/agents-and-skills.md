...
These commands accept `--json` for automation and `--format` for shell-friendly output. The doctor command also accepts `--strict` (or `--strict-runtime`) to promote invalid or unavailable agent runtime defaults to failures. The plural alias also works:
