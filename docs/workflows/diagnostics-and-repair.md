...
resume, or log fallback. Overview and monitor JSON/text, plus status summary
text and enriched status-summary JSON, also count runtime resume capabilities
as `managed_resume`, `can_managed_resume`, and `direct_resume`, so dashboards
can tell whether recovery can stay daemon-managed or needs a direct runtime
fallback before running a detailed `resume-plan`.
...
Add `--fallbacks` to `overview`, `team overview`, `next`, `team next`,
`health`, or `monitor` when runtime recovery hints should hand off to
`resume-plan --commands --fallbacks` instead of the single recommended
resume-plan row.
...
Add `--fallbacks` when runtime recommendations should call resume-plan with
...
final-message fallbacks into `resume-plan` follow-ups. Add `--fallbacks` when
...
agent-team monitor --summary --commands
...
| Need raw runtime metadata | `agent-team runtime metadata ls --summary --json` |
| Need one raw runtime metadata record | `agent-team runtime metadata show worker-squ-42 --json` |
...
| Need saved artifact versus current state for the same scope | `agent-team snapshot diff before.json --current-after` |
