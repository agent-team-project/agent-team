...
When a whole job should stop advancing without changing its lifecycle status, use `agent-team job hold <job-id> [reason...]`. Add `--for 2h` or `--until 2026-06-24T18:00:00Z` when the pause has a known deadline. Held jobs report next-step state `held`, are skipped by default ready and advance loops, and remain visible through `job ready --state held`, `pipeline explain --state held`, and `team explain --state held`. Use `agent-team job release <job-id> [message...]` to resume normal readiness checks; expired holds stay paused until released.
