...
Use `agent-team team approve <team>` for the same manual-gate approval flow scoped to one team's declared pipelines, and `agent-team team reject <team> --dry-run` before rejecting the team's blocked manual gates.
