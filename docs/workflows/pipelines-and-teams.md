...
Use `agent-team pipeline send <pipeline> --dry-run "..."` when every running instance owned by jobs in one workflow should receive the same operator instruction. Add `--runtime codex`, `--latest`, `--last N`, or `--all` to narrow or widen recipients before sending. Use `agent-team pipeline logs <pipeline>` to read those same pipeline-owned runtime logs; add `--last-message` for clean Codex final responses, or `--list --json` when scripts need log paths.
