...
agent-team daemon adopt <instance> --pid <pid> [--agent manager] [--workspace <path>] [--runtime claude|codex] [--session-id <id>] [--job squ-42] [--dry-run] [--format '{{.Metadata.Instance}} {{.Metadata.PID}}'] [--json]
...
Shortcuts: `agent-team up` = `start`, `agent-team down` = `stop`, `agent-team ls` = `ps`, and `agent-team top` = `stats`. Collection command groups also accept plural aliases: `jobs`, `pipelines`, `queues`, `schedules`, and `teams`.
