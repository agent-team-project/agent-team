...
`pipeline retry --dispatch` and `team retry --dispatch` also accept `--wait`, `--wait-status`, `--wait-event`, `--wait-timeout`, and `--fail-on-failed` for bounded failed-step recovery handoffs.
