...
Use `monitor --jobs --schedules` or `job triage` plus `schedule next` to inspect what needs attention. `job triage` and `monitor --jobs` include durable job triage plus status-file job update previews, so unreconciled blocked workers are visible before a maintenance tick; `job show <id>` shows the matching queue and status previews for one job. Add `health --jobs` in scripts when stuck/failed jobs should make the health check fail too; it also previews status-file job updates and fails if an unreconciled worker status reports a blocked job. Use `job reconcile status` when workers have written `status.toml` and you want to refresh the owning durable jobs without dispatching anything. Use `tick` to act on ready work: it reconciles stale daemon metadata and job status files, fires due schedules, asks the daemon to dispatch ready queued events, and advances ready pipeline jobs. `tick --dry-run` previews job-status, schedule, queue, and pipeline work without mutating state; `tick --watch` repeats the cycle in the foreground; `tick --until-idle --max-cycles N` is the finite CI/script mode. `--json` emits one JSON object per cycle for watch mode, or an aggregate cycle result for until-idle mode.
...
Use `job unblock <job-id> <answer...>` when a blocked worker needs operator input: it accepts blocked status-file previews from `job triage` or `job show`, sends the answer to the owning instance, marks the durable job running, and records an audit event. Add `--dry-run` to preview the target and status transition without sending a mailbox message. Use `job retry <job-id> --dispatch` for the common failed-job recovery path: it records a reopen event, then immediately sends the job back through daemon dispatch. For pipeline jobs, it resets the first failed step whose dependencies are satisfied, then advances the next ready step.
