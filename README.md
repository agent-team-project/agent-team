...
Run `agent-team runtime` to confirm the selected profile, resolved binary path, config source, and supported capabilities. Use `agent-team runtime probe --runtime codex --json` before dispatching Codex work when you need one screen that combines agent-team runtime selection, daemon socket readiness, and `codex doctor` reachability/auth/sandbox checks. Add `--exec --timeout 2m` when you also want a real Codex `exec -` smoke test that verifies stdin prompt handling and last-message sidecar capture, or add `--output runtime-probe.json` to preserve the full diagnostic artifact.
