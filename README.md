...
agent-team pipeline queue <pipeline> [--state dead] [--job SQU-42] [--runtime codex] [--summary] [--sort attempts] [--limit N] [--format '{{.ID}} {{.State}}'] [--json] | queue show <pipeline> <id> [--format '{{.ID}} {{.State}}'] [--json] | queue retry|drop <pipeline> <id>|--all [--runtime codex] [--dry-run] [--job SQU-42] [--format '{{.ID}} {{.Action}}'] [--json] | queue prune <pipeline> [--state dead] [--runtime codex] [--dry-run] [--format '{{.ID}} {{.Dropped}}'] [--json]
                                                # inspect, retry, drop, or prune active queue items owned by one pipeline
...
Pipeline status rows also support `--sort jobs --limit 10` on `pipeline status --all` and `team pipelines <team>` when a large workspace needs the busiest workflows first.
