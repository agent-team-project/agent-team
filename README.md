...
agent-team snapshot [--events N|-1] [--intake-deliveries N|-1] [--schedule-limit N] [--no-redact] [--json | --output snapshot.json] | diff <before.json> <after.json> [--section instances|jobs|queue|pipelines|advance|section_errors] [--json] [--exit-code]
