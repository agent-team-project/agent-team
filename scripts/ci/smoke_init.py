...
def check_daemon_lifecycle(binary: Path, target: Path) -> list[str]:
    ...
    try:
        ...
        try:
            daemon_start_body = json.loads(r.stdout)
        except Exception as e:  # noqa: BLE001
            problems.append(f"daemon start --json returned invalid JSON: {e}\nstdout={r.stdout}\nstderr={r.stderr}")
            daemon_start_body = {}
        ...
        try:
            direct_daemon_status = json.loads(r.stdout)
        except Exception as e:  # noqa: BLE001
            problems.append(f"daemon status --wait --json after daemon start returned invalid JSON: {e}\nstdout={r.stdout}\nstderr={r.stderr}")
            direct_daemon_status = {}
        ...
        try:
            daemon_stop_body = json.loads(r.stdout)
        except Exception as e:  # noqa: BLE001
            problems.append(f"daemon stop --json after direct daemon start returned invalid JSON: {e}\nstdout={r.stdout}\nstderr={r.stderr}")
            daemon_stop_body = {}
        ...
        try:
            stopped_daemon_status = json.loads(r.stdout)
        except Exception as e:  # noqa: BLE001
            problems.append(f"daemon status --wait --down --json after daemon stop returned invalid JSON: {e}\nstdout={r.stdout}\nstderr={r.stderr}")
        ...
        try:
            dry_start_rows = json.loads(r.stdout)
        except Exception as e:  # noqa: BLE001
            problems.append(f"start --dry-run --json before daemon returned invalid JSON: {e}\nstdout={r.stdout}\nstderr={r.stderr}")
            dry_start_rows = []
        ...
        if r.returncode != 0 or formatted_plan_rows != ["feedback-triage:on-demand", "manager:start"]:
        ...
        if r.returncode != 0 or formatted_filtered_sync_rows != ["feedback-triage:on-demand:unknown", "manager:start:unknown"]:
        ...
        elif filtered_plan_before_summary.get("total") != 2 or filtered_plan_before_summary.get("start") != 1 or filtered_plan_before_summary.get("on_demand") != 1:
            problems.append(f"filtered plan --json before start returned unexpected summary: {filtered_plan_before_start}")
        elif [row.get("instance") for row in filtered_plan_before_rows] != ["feedback-triage", "manager"]:
