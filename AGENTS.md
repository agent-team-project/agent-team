...
3. On `agent-team run <agent>`, launches Codex with the vendored team registered for the session, via Codex's `--agents` and `--add-dir` flags.
...
- **workspace** — an instance's working directory. For ephemeral code-writing agents: a fresh worktree per spawn (Codex's `Agent` tool with `isolation: "worktree"`). For others: the repo root.
...
- [`documentation/orchestrator.md`](./documentation/orchestrator.md) — v1.1+ Go daemon (`agent-teamd`) that owns instance lifecycle, replaces Codex's in-session dispatch primitives with an orchestrator-mediated model, and unblocks runtime-agnostic execution. Read before touching the dispatch path or thinking about persistent / restartable instances.
...
The CLI assembles `{name: {description, prompt}, …}` as JSON, builds a tmpdir with `.Codex/skills/<name>` symlinks for the union of all referenced skills, writes the chosen agent's prompt + a kickoff preamble (instance name, state dir) to a temp file, creates `.agent_team/state/<instance>/` if missing, and exec's:

```sh
Codex --agents '<json>' --add-dir <tmpdir> --append-system-prompt-file <kickoff> <forwarded-args>
```
...
The launcher exports into Codex's env:
...
Skills are picked up by Codex's `--add-dir` discovery — see [Skills docs](https://code.Codex.com/docs/en/skills) for the directory shape `--add-dir` expects.
...
- Trailer: `Co-authored-by: Codex Opus 4.7 (1M context) <noreply@anthropic.com>` on any commit an agent helped author.

### PR body

Lead with a short summary of what changed and why. Link the ticket via `Closes https://linear.app/squirtlesquad/issue/SQU-<n>/<slug>`. End with the standard Codex footer.
